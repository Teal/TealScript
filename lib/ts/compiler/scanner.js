...
(function (ts) {
        ...
        while (pos < text.length) {
            ...
            switch (ch) {
                ...
                case ts.CharCode.lineFeed:
                    ...
                    break;
                default:
                    if (ch > ts.CharCode.asciiMax && ts.isLineBreak(ch)) {
                        result.push(lineStart);
                        lineStart = pos;
                    }
                    break;
        ...
        while (true) {
            ...
            switch (ch) {
                ...
                case ts.CharCode.hash:
                    ...
                    break;
                default:
                    if (ch > ts.CharCode.asciiMax && (ts.isWhiteSpace(ch))) {
                        pos++;
                        continue;
                    }
                    break;
    ...
    function createScanner(languageVersion, skipTrivia, languageVariant, text, start, length) {
        ...
        return {
            ...
            isIdentifier: function () { return token === 215 /* Identifier */ || token > 251 /* LastReservedWord */; },
            isReservedWord: function () { return token >= 216 /* FirstReservedWord */ && token <= 251 /* LastReservedWord */; },
