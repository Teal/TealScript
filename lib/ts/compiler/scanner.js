...
(function (ts) {
    ...
    function scanConflictMarkerTrivia(text, pos) {
        if (ts.error) {
            ts.error(ts.Diagnostics.Merge_conflict_marker_encountered, mergeConflictMarkerLength);
        }
        var ch = text.charCodeAt(pos);
        var len = text.length;
    ...
    function createScanner(languageVersion, skipTrivia, languageVariant, text, start, length) {
        ...
        function scanNumber() {
            var start = pos;
            while (ts.isDecimalDigit(text.charCodeAt(pos)))
                pos++;
            ...
            if (text.charCodeAt(pos) === ts.CharCode.E || text.charCodeAt(pos) === ts.CharCode.e) {
                ...
                if (text.charCodeAt(pos) === ts.CharCode.plus || text.charCodeAt(pos) === ts.CharCode.minus)
                    pos++;
                if (ts.isDecimalDigit(text.charCodeAt(pos))) {
                    pos++;
                    while (ts.isDecimalDigit(text.charCodeAt(pos)))
                        pos++;
                    end = pos;
                    ...
                    ts.error(ts.Diagnostics.Digit_expected);
                }
            }
        ...
        function scanString() {
            ...
            while (true) {
                if (pos >= end) {
                    ...
                    ts.error(ts.Diagnostics.Unterminated_string_literal);
                    break;
                }
                var ch = text.charCodeAt(pos);
                if (ch === quote) {
        ...
        function scanTemplateAndSetTokenValue() {
            ...
            while (true) {
                if (pos >= end) {
                    ...
                    tokenIsUnterminated = true;
                    ts.error(ts.Diagnostics.Unterminated_template_literal);
        ...
        function scanEscapeSequence() {
            pos++;
            if (pos >= end) {
                ts.error(ts.Diagnostics.Unexpected_end_of_text);
                return "";
            }
            var ch = text.charCodeAt(pos);
            pos++;
        ...
        function scanHexadecimalEscape(numDigits) {
            ...
            if (escapedValue >= 0) {
                ...
                ts.error(ts.Diagnostics.Hexadecimal_digit_expected);
                return "";
            }
        }
        function scanExtendedUnicodeEscape() {
            ...
            if (escapedValue < 0) {
                ...
                ts.error(ts.Diagnostics.An_extended_Unicode_escape_value_must_be_between_0x0_and_0x10FFFF_inclusive);
                isInvalidExtendedEscape = true;
            }
            if (pos >= end) {
                ts.error(ts.Diagnostics.Unexpected_end_of_text);
                ...
                ts.error(ts.Diagnostics.Unterminated_Unicode_escape_sequence);
                isInvalidExtendedEscape = true;
            }
        ...
        function scanBinaryOrOctalDigits(base) {
            ...
            while (true) {
                ...
                if (!ts.isDecimalDigit(ch) || valueOfCh >= base) {
                    break;
                }
            ...
            while (true) {
                ...
                switch (ch) {
                    ...
                    case ts.CharCode.dot:
                        if (ts.isDecimalDigit(text.charCodeAt(pos + 1))) {
                    ...
                    case ts.CharCode.slash:
                        ...
                        if (text.charCodeAt(pos + 1) === ts.CharCode.asterisk) {
                            ...
                            if (!commentClosed) {
                                ts.error(ts.Diagnostics.Asterisk_Slash_expected);
                            }
                            if (skipTrivia) {
                                continue;
                            }
                    ...
                    case ts.CharCode.num0:
                        if (pos + 2 < end && (text.charCodeAt(pos + 1) === ts.CharCode.X || text.charCodeAt(pos + 1) === ts.CharCode.x)) {
                            ...
                            if (value < 0) {
                                ts.error(ts.Diagnostics.Binary_digit_expected);
                                value = 0;
                            }
                            tokenValue = "" + value;
                            return token = 154 /* NumericLiteral */;
                            ...
                            if (value < 0) {
                                ts.error(ts.Diagnostics.Octal_digit_expected);
                                value = 0;
                            }
                            tokenValue = "" + value;
                            return token = 154 /* NumericLiteral */;
                    ...
                    case ts.CharCode.lessThan:
                        if (isConflictMarkerTrivia(text, pos)) {
                            pos = scanConflictMarkerTrivia(text, pos, ts.error);
                            if (skipTrivia) {
                                continue;
                            }
                            else {
                    ...
                    case ts.CharCode.backslash:
                        ...
                        ts.error(ts.Diagnostics.Invalid_character);
                        pos++;
        ...
        function reScanSlashToken() {
            if (token === 46 /* slash */ || token === 81 /* slashEquals */) {
                ...
                while (true) {
                    ...
                    if (p >= end) {
                        tokenIsUnterminated = true;
                        ts.error(ts.Diagnostics.Unterminated_regular_expression_literal);
                        break;
                    }
