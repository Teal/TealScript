        ...
        TestState.prototype.verifyCompletionListDoesNotContain = function (symbol, expectedText, expectedDocumentation, expectedKind) {
            ...
            if (completions) {
                ...
                if (filterCompletions.length !== 0) {
                    ...
                    // raise an error
                    var error_1 = "Completion list did contain \'" + symbol + "\'.";
                    var details = this.getCompletionEntryDetails(filterCompletions[0].name);
                    if (expectedText) {
                        error_1 += "Expected text: " + expectedText + " to equal: " + ts.displayPartsToString(details.displayParts) + ".";
                    }
                    if (expectedDocumentation) {
                        error_1 += "Expected documentation: " + expectedDocumentation + " to equal: " + ts.displayPartsToString(details.documentation) + ".";
                    }
                    if (expectedKind) {
                        error_1 += "Expected kind: " + expectedKind + " to equal: " + filterCompletions[0].kind + ".";
                    }
                    this.raiseError(error_1);
                }
            }
        };
