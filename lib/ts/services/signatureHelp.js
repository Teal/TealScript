...
(function (ts) {
    ...
    (function (SignatureHelp) {
        ...
        function createJavaScriptSignatureHelpItems(argumentInfo, program) {
            ...
            var name = expression.kind === SyntaxKind.Identifier
                ? expression
        ...
        function getImmediatelyContainingArgumentInfo(node, position, sourceFile) {
            if (node.parent.kind === SyntaxKind.CallExpression || node.parent.kind === SyntaxKind.NewExpression) {
                ...
                if (node.kind === SyntaxKind.LessThanToken ||
                    ...
                    var list_1 = getChildListThatStartsWithOpenerToken(callExpression, node, sourceFile);
                    var isTypeArgList = callExpression.typeArguments && callExpression.typeArguments.pos === list_1.pos;
                    ts.Debug.assert(list_1 !== undefined);
                    return {
                        kind: isTypeArgList ? 0 /* TypeArguments */ : 1 /* CallArguments */,
                        invocation: callExpression,
                        argumentsSpan: getApplicableSpanForArguments(list_1, sourceFile),
                        argumentIndex: 0,
                        argumentCount: getArgumentCount(list_1)
                    };
                }
                ...
                if (listItemInfo) {
                    var list_2 = listItemInfo.list;
                    var isTypeArgList = callExpression.typeArguments && callExpression.typeArguments.pos === list_2.pos;
                    var argumentIndex = getArgumentIndex(list_2, node);
                    var argumentCount = getArgumentCount(list_2);
                    ts.Debug.assert(argumentIndex === 0 || argumentIndex < argumentCount, "argumentCount < argumentIndex, " + argumentCount + " < " + argumentIndex);
                    return {
                        kind: isTypeArgList ? 0 /* TypeArguments */ : 1 /* CallArguments */,
                        invocation: callExpression,
                        argumentsSpan: getApplicableSpanForArguments(list_2, sourceFile),
                        argumentIndex: argumentIndex,
                ...
                ts.Debug.assert(templateExpression.kind === SyntaxKind.TemplateExpression);
                ...
                if (node.kind === SyntaxKind.TemplateTail && !ts.isInsideTemplateLiteral(node, position)) {
                    return undefined;
                }
        ...
        function getArgumentCount(argumentsList) {
            ...
            // arg count of 3.
            var listChildren = argumentsList.getChildren();
            var argumentCount = ts.countWhere(listChildren, function (arg) { return arg.kind !== SyntaxKind.CommaToken; });
            if (listChildren.length > 0 && ts.lastOrUndefined(listChildren).kind === SyntaxKind.CommaToken) {
                argumentCount++;
            }
        ...
        function createSignatureHelpItems(candidates, bestSignature, argumentListInfo, typeChecker) {
            ...
            var items = ts.map(candidates, function (candidateSignature) {
                ...
                if (isTypeParameterList) {
                    prefixDisplayParts.push(ts.punctuationPart(SyntaxKind.LessThanToken));
                    ...
                    suffixDisplayParts.push(ts.punctuationPart(SyntaxKind.GreaterThanToken));
                    ...
                    prefixDisplayParts.push(ts.punctuationPart(SyntaxKind.OpenParenToken));
                    ...
                    suffixDisplayParts.push(ts.punctuationPart(SyntaxKind.CloseParenToken));
                }
                ...
                return {
                    ...
                    separatorDisplayParts: [ts.punctuationPart(SyntaxKind.CommaToken), ts.spacePart()],
