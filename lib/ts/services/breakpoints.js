        ...
        function spanInSourceFileAtLocation(sourceFile, position) {
            ...
            function spanInNode(node) {
                ...
                function spanInBindingPattern(bindingPattern) {
                    ...
                    var firstBindingElement = ts.forEach(bindingPattern.elements, function (element) { return element.kind !== SyntaxKind.OmittedExpression ? element : undefined; });
                ...
                function spanInArrayLiteralOrObjectLiteralDestructuringPattern(node) {
                    ts.Debug.assert(node.kind !== SyntaxKind.ArrayBindingPattern && node.kind !== SyntaxKind.ObjectBindingPattern);
                    var elements = node.kind === SyntaxKind.ArrayLiteralExpression ?
                        node.elements :
                        node.properties;
                    var firstBindingElement = ts.forEach(elements, function (element) { return element.kind !== SyntaxKind.OmittedExpression ? element : undefined; });
                    if (firstBindingElement) {
                        return spanInNode(firstBindingElement);
                    }
