...
解析表达式
    * name;
"minPrecedence" > 当前解析的最低操作符优先级;
/param>
    * /returns>;
Expression;
parseExpression(int, minPrecedence = 0);
{
    // Expression :
    //   UnaryExpression
    // UnaryExpression :
    ...
    switch (type) {
        ...
        case tokenType_1.TokenType.stringLiteral:
            ...
            break;
        // 0
        case tokenType_1.TokenType.intLiteral:
            parsed = parseIntOrLongLiteral(lexer_1.Lexer.parseLongToken(lexer.peek()));
            break;
        // 0x0
        case tokenType_1.TokenType.hexIntLiteral:
            parsed = parseIntOrLongLiteral(lexer_1.Lexer.parseHexIntToken(lexer.peek()));
            break;
        // .0
        case tokenType_1.TokenType.floatLiteral:
            parsed = new FloatLiteral();
            {
                startLocation = lexer.read().startLocation,
                    value = lexer_1.Lexer.parseFloatToken(lexer.current),
                    endLocation = lexer.current.endLocation;
            }
            ;
            break;
...
/param>
    * /returns>;
Expression;
parseExpression(Expression, parsed, int, minPrecedence = 0);
{
    // PostfixExpression :
    //   MemberExpression
...
parseExpression();
{
    ...
    var result_8 = this.parseAssignmentExpressionOrHigher();
    while (this.readToken(tokenType_1.TokenType.comma)) {
        result_8 = this.makeBinaryExpression(result_8, tokenType_1.TokenType.comma, this.lexer.tokenStart - 1, this.parseAssignmentExpressionOrHigher());
    }
    return result_8;
}
...
parseYieldExpression();
{
    ...
    var result_9 = new nodes.YieldExpression();
    result_9.start = this.lexer.read().start; // yield
    if (!this.lexer.currentToken.hasLineTerminatorBeforeStart) {
        if (this.lexer.tokenType === tokenType_1.TokenType.nodeserisk) {
            result_9.nodeseriskStart = this.lexer.read().start;
            result_9.value = this.parseAssignmentExpressionOrHigher();
        }
...
{
    var result_10 = new nodes.BinaryExpression();
    result_10.leftOperand = left;
    result_10.operator = operator;
    result_10.operatorStart = operatorStart;
    result_10.rightOperand = right;
    return result_10;
}
...
JsxElement | JsxSelfClosingElement;
{
    ...
    if (inExpressionContext && token === tokenType_1.TokenType.LessThanToken) {
        ...
        if (invalidElement) {
            ...
            var badNode = createNode(tokenType_1.TokenType.BinaryExpression, result_11.pos);
            badNode.end = invalidElement.end;
            badNode.left = result_11;
...
{
    this.expectToken(tokenType_1.TokenType.OpenParenToken);
    var result_12 = parseDelimitedList(ParsingContext.ArgumentExpressions, parseArgumentExpression);
    this.expectToken(tokenType_1.TokenType.CloseParenToken);
    return result_12;
}
...
parseCondition();
{
    ...
    if (this.readToken(tokenType_1.TokenType.openParen)) {
        result_13 = this.parseExpression(0);
        ...
        result_13 = this.parseExpression();
    }
    return result_13;
}
parseEmbeddedStatement();
...
parseDoWhileStatement();
{
    // DoWhileStatement :
    //   do EmbeddedStatement while Condition ;
    var result_14 = new nodes.DoWhileStatement();
    ...
    result_14.end = this.lexer.tokenEnd;
    return result_14;
}
parseWhileStatement();
{
    // WhileStatement :
    //   while Condition EmbeddedStatement ;
    var result_15 = new nodes.WhileStatement();
    result_15.start = this.lexer.tokenStart;
    this.expectToken(tokenType_1.TokenType.while);
    result_15.condition = this.parseCondition();
    result_15.body = this.parseEmbeddedStatement();
    return result_15;
}
parseForStatement();
{
...
parseBreakStatement();
{
    // BreakStatement :
    //   break ;
    var result_16 = new nodes.BreakStatement();
...
parseContinueStatement(kind, tokenType_1.TokenType);
{
    // ContinueStatement :
    //   continue ;
    var result_17 = new nodes.ContinueStatement();
    result_17.start = this.lexer.tokenStart;
    this.expectToken(tokenType_1.TokenType.continue);
    if (!this.autoInsertSemicolon()) {
        result_17.label = this.parseIdentifier();
    }
    this.readToken(tokenType_1.TokenType.semicolon);
    result_17.end = this.lexer.tokenEnd;
    return result_17;
}
parseReturnStatement();
{
    // ReturnStatement :
    //   return Expression? ;
    var result_18 = new nodes.ReturnStatement();
    result_18.start = this.lexer.tokenStart;
    this.expectToken(tokenType_1.TokenType.return);
    if (!this.autoInsertSemicolon()) {
        result_18.value = this.parseExpression(true);
    }
    parseSemicolon();
    return finishNode(node);
}
...
CatchClause;
{
    var result_19 = createNode(tokenType_1.TokenType.CatchClause);
    this.expectToken(tokenType_1.TokenType.CatchKeyword);
    if (this.expectToken(tokenType_1.TokenType.OpenParenToken)) {
        result_19.variableDeclaration = parseVariableDeclaration();
    }
    this.expectToken(tokenType_1.TokenType.CloseParenToken);
    result_19.block = parseBlock(/*ignoreMissingOpenBrace*/ false);
    return finishNode(result_19);
}
...
parseDecorators();
{
    var result_20 = new nodes.NodeList();
    do {
...
{
    if (token === tokenType_1.TokenType.SemicolonToken) {
        var result_21 = createNode(tokenType_1.TokenType.SemicolonClassElement);
        nextToken();
        return finishNode(result_21);
    }
    var fullStart = getNodePos();
    var decorators = parseDecorators();
...
{
    if (token === tokenType_1.TokenType.StringLiteral) {
        var result_22 = parseLiteralNode();
        internIdentifier(result_22.text);
        return result_22;
    }
    else {
...
(function (IncrementalParser) {
    ...
    SourceFile;
    {
        ...
        // are already correct.
        var result_24 = Parser.parseSourceFile(sourceFile.fileName, newText, sourceFile.languageVersion, syntaxCursor, /*setParentNodes*/ true, sourceFile.scriptKind);
        return result_24;
    }
