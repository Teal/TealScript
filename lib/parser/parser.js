...
var Parser = (function () {
    ...
    Parser.prototype.readToken = function (token) {
        ...
        this.error(this.lexer.peek(), token !== tokenType_1.TokenType.identifier ? "应输入“{0}”。" : tokenType_1.isKeyword(token) ? "应输入标识符；“{0}”是关键字。" : "应输入标识符。", tokenType_1.tokenToString(token));
     ...
     */
    Parser.prototype.parseSimpleList = function (parseElement) {
     ...
     */
    Parser.prototype.parseDelimitedList = function (openToken, parseElement, closeToken) {
        var result = new nodes.NodeList();
        result.start = this.readToken(openToken);
        while (this.lexer.peek().type !== closeToken && this.lexer.peek().type !== tokenType_1.TokenType.endOfFile) {
            var element = parseElement.call(this);
            if (!element)
                break;
            result.push(element);
    ...
    Parser.prototype.tryParse = function (parser) {
        ...
        var result = parser.call(this, errors);
    ...
    Parser.prototype.parseTypeNode = function () {
        ...
        switch (this.lexer.peek().type) {
            case tokenType_1.TokenType.openParen:
                var parameters = this.tryParseParameterDeclarations();
                if (parameters) {
     ...
     * 尝试解析一个参数定义列表。
     */
    Parser.prototype.tryParseParameterDeclarations = function () {
        var _this = this;
        return this.tryParse(function () {
            var parameters = _this.parseParameterDeclarations();
    ...
    Parser.prototype.parseTypeParameterDeclarations = function () {
        if (this.lexer.peek().type === tokenType_1.TokenType.lessThan) {
            return this.parseDelimitedList(tokenType_1.TokenType.lessThan, this.parseTypeParameterDeclaration, tokenType_1.TokenType.greaterThan);
        }
    };
    /**
    ...
    Parser.prototype.parseTypeParameterDeclaration = function () {
        // 当前必须是 < 或 , 才是类型参数开始。
        if (this.lexer.current.type !== tokenType_1.TokenType.comma &&
            this.lexer.current.type !== tokenType_1.TokenType.lessThan)
            return;
        var result = new nodes.TypeParameterDeclaration();
        result.name = this.parseIdentifier();
        if (this.tryReadToken(tokenType_1.TokenType.extends)) {
    ...
    Parser.prototype.parseParameterDeclarations = function (yieldContext, awaitContext, requireCompleteParameterList) {
        // todo: 设置 yield  和 await
        return this.parseDelimitedList(tokenType_1.TokenType.openParen, this.parseParameterDeclaration, tokenType_1.TokenType.closeParen);
    ...
    Parser.prototype.parseParameterDeclaration = function () {
        ...
        result.dotDotDotToken = this.tryReadTokenToken(tokenType_1.TokenType.dotDotDot);
        ...
        result.questionToken = this.tryReadTokenToken(tokenType_1.TokenType.question);
        result.type = this.parseParameterType();
    ...
    Parser.prototype.parseDecorator = function () {
        if (!this.tryReadToken(tokenType_1.TokenType.at))
            return;
    ...
    Parser.prototype.parseUnionOrIntersectionTypeOrHigher = function (type) {
        var result = type === tokenType_1.TokenType.ampersand ? this.parseArrayTypeOrHigher() : this.parseUnionOrIntersectionTypeOrHigher(tokenType_1.TokenType.ampersand);
        while (this.lexer.peek().type === type) {
            var newResult = type === tokenType_1.TokenType.ampersand ? new nodes.IntersectionTypeNode() : new nodes.UnionTypeNode();
            newResult.leftOperand = result;
            newResult.operatorToken = this.readToken(type);
            newResult.rightOperand = type === tokenType_1.TokenType.ampersand ? this.parseArrayTypeOrHigher() : this.parseUnionOrIntersectionTypeOrHigher(tokenType_1.TokenType.ampersand);
            result = newResult;
        }
        return result;
    };
    ...
    Parser.prototype.parseTypeAnnotation = function () {
        return this.tryReadToken(tokenType_1.TokenType.colon) ? this.parseTypeNode() : undefined;
    };
    // #endregion
    // #region 解析表达式
    /**
    ...
    Parser.prototype.parseConditionalExpressionRest = function (leftOperand) {
        ...
        result.colonToken = this.readTokenToken(tokenType_1.TokenType.colon, /*reportAtCurrentPosition*/ false, nodes.Diagnostics._0_expected, tokenType_1.tokenToString(tokenType_1.TokenType.colon));
    ...
    Parser.prototype.parseSwitchStatement = function () {
        ...
        result.start = this.lexer.read().start;
        if (!this.options.disallowMissingSwitchCondition || this.lexer.peek().type !== tokenType_1.TokenType.openBrace) {
            this.parseCondition(result);
        }
        ...
        while (true) {
            ...
            if (this.lexer.current.type === tokenType_1.TokenType.case) {
                if (!this.options.disallowCaseElse && this.lexer.peek().type === tokenType_1.TokenType.else) {
    ...
    Parser.prototype.nextTokenCanFollowModifier = function () {
        if (this.lexer.peek().type === tokenType_1.TokenType.const) {
            ...
            return this.lexer.read().type === tokenType_1.TokenType.enum;
        }
    ...
    Parser.prototype.parseTemplateSpan = function () {
        ...
        if (this.lexer.peek().type === tokenType_1.TokenType.closeBrace) {
            ...
            literal = this.readTokenToken(tokenType_1.TokenType.TemplateTail, /*reportAtCurrentPosition*/ false, nodes.Diagnostics._0_expected, tokenType_1.tokenToString(tokenType_1.TokenType.closeBrace));
        }
    ...
    Parser.prototype.parseSourceFile = function (text, start, fileName) {
        ...
        if (this.lexer.comments) {
            for (var _i = 0, _a = this.lexer.comments; _i < _a.length; _i++) {
                var comment = _a[_i];
                // 只处理 /// 开头的注释。
                if (this.lexer.source.charCodeAt(comment.start) !== unicode_1.CharCode.slash ||
                    this.lexer.source.charCodeAt(comment.start - 1) !== unicode_1.CharCode.slash) {
                    continue;
                }
            }
        }
