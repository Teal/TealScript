...
var this_lexer_1 = require('./this.lexer');
 ...
 */
this.lexer = new this_lexer_1.Lexer();
/**
 * 解析指定的源文件。
 * @param text 要解析的源码。
 ...
 * @param args 格式化错误的参数。
 */
compiler_1.error.apply(void 0, [token, this_lexer_1.Token, message, string].concat(args, [any[]]));
{
...
flags: ParseFlags = 0;
readToken(token, tokenType_1.TokenType);
{
...
expectToken(token, tokenType_1.TokenType);
{
    if (this.lexer.peek().type === token) {
 ...
 */
parseNodeList(start, tokenType_1.TokenType, parseElement, function () { return T; }, end, tokenType_1.TokenType);
{
    var result_1 = new nodes.NodeList();
    return result_1;
}
autoInsertSemicolon();
{
...
parseBlockStatement();
{
    ...
    var result_2 = new nodes.BlockStatement();
    result_2.start = this.lexer.read().start;
    result_2.statements = this.parseStatementList(tokenType_1.TokenType.closeBrace);
    result_2.end = this.lexer.read().end;
    console.assert(this.lexer.current.type === tokenType_1.TokenType.closeBrace);
    return result_2;
}
parseStatementList(end ?  : tokenType_1.TokenType);
{
    ...
    var result_3 = [];
    while (true) {
        result_3.push(this.parseStatement());
        switch (this.lexer.peek().type) {
            case end:
                return result_3;
            case tokenType_1.TokenType.endOfFile:
...
parseVariableStatement(type, Function, decorators ?  : nodes.NodeList < nodes.Decorator > , modifiers ?  : nodes.NodeList(), {
    ...
    const: result = this.lexer.peek().type === tokenType_1.TokenType.var ? new nodes.VarStatement() :
...
private, parseVariableDeclarationList(), {
    ...
    }, while: function () { }, this: .readToken(tokenType_1.TokenType.comma) });
return result;
parseVariableDeclaration();
{
    ...
    var result_4 = new nodes.VariableDeclaration();
    result_4.name = this.parseBindingIdentifierOrPattern(); // BindingIdentifier | BindingPattern
    if (this.readToken(tokenType_1.TokenType.colon)) {
        result_4.colonStart = this.lexer.current.start;
        result_4.type = this.parseTypeExpression();
    }
    if (this.readToken(tokenType_1.TokenType.equals)) {
        result_4.equalStart = this.lexer.current.start;
        result_4.initialiser = this.parseExpression();
    }
    return result_4;
}
...
parseEmptyStatement();
{
    ...
    var result_5 = new nodes.EmptyStatement();
    result_5.start = this.lexer.read().start;
    return result_5;
}
parseLabeledStatement(label, nodes.Identifier);
{
    // LabeledStatement :
    //   Identifier : Statement
    console.assert(this.lexer.current.type === tokenType_1.TokenType.colon);
    var result_6 = new nodes.LabeledStatement();
    result_6.label = label;
    result_6.colonStart = this.lexer.read().type;
    result_6.body = this.parseStatement();
    return result_6;
}
parseExpressionStatement();
{
    ...
    var result_7 = new nodes.ExpressionStatement();
    result_7.body = this.parseExpression();
    result_7.end = this.autoInsertSemicolon();
    return result_7;
}
parseIfStatement();
{
    ...
    var result_8 = new nodes.IfStatement();
    result_8.start = this.lexer.read().start;
    result_8.condition = this.parseCondition();
    result_8.then = this.parseEmbeddedStatement();
    if (this.readToken(tokenType_1.TokenType.else)) {
        result_8.else = this.parseEmbeddedStatement();
    }
    return result_8;
}
parseSwitchStatement();
{
    ...
    var result_9 = new nodes.SwitchStatement();
    result_9.start = this.lexer.read().start; // switch
    result_9.condition = this.lexer.peek().type === tokenType_1.TokenType.openBrace ? undefined : this.parseCondition();
    result_9.cases = this.parseNodeList(tokenType_1.TokenType.openBrace, this.parseCaseClause, tokenType_1.TokenType.closeBrace);
    return result_9;
}
parseCaseClause();
{
    ...
    var result_10 = new nodes.CaseClause();
    result_10.start = this.lexer.read().start;
    if (this.lexer.current.type === tokenType_1.TokenType.case) {
        result_10.label = this.parseExpressionWith(ParseContext.allowElse);
    }
    result_10.colonStart = this.expectToken(tokenType_1.TokenType.colon);
    result_10.statements = this.parseNodeList(null, this.parseStatement, tokenType_1.TokenType.closeBrace);
    return result_10;
}
parseForStatement();
{
...
parseForStatement(Location, start, bool, hasParam, Node, initializer);
{
...
parseForInStatement(Location, start, bool, hasParam, Variable, variable);
{
...
parseForOfStatement(node, nodes.ForOfStatement);
{
}
parseForToStatement(Location, start, bool, hasParam, Variable, variable);
{
    if (variable.initialiser == null) {
...
parseDoWhileStatement();
{
    ...
    expectToken(tokenType_1.TokenType.while, ErrorCode.expectedWhile);
    result.start = start;
    result.body = body;
...
parseExpression(minPrecedence ?  : number);
{
    console.assert(tokenType_1.isExpressionStart(this.lexer.peek().type));
    var parsed_1;
    var type_1 = this.lexer.peek().type;
    switch (type_1) {
        // Identifier、Identifier<T>、Identifier[]
        case tokenType_1.TokenType.identifier:
            parsed_1 = this.parseRestTypeExprssion(this.parseIdentifier());
            break;
        // (Expr)、(Expr) => {...}
        case tokenType_1.TokenType.openParen:
            parsed_1 = this.parseParenthesizedExpressionOrArrowFunction();
            break;
        // new Expr
        case tokenType_1.TokenType.new:
            parsed_1 = this.parseNewExpression();
            break;
        // ""、''
        case tokenType_1.TokenType.stringLiteral:
            parsed_1 = this.parseStringLiteral();
            break;
        // 0
        case tokenType_1.TokenType.numericLiteral:
            parsed_1 = this.parseNumericLiteral();
            break;
        // [Expr, ...]
        case tokenType_1.TokenType.openBracket:
            parsed_1 = this.parseArrayLiteral();
            break;
        // {key: Expr, ...}
        case tokenType_1.TokenType.openBrace:
            parsed_1 = this.parseObjectLiteral();
            break;
        // @ Identifier
        case tokenType_1.TokenType.at:
            parsed_1 = this.parseAtExpression();
            break;
        case tokenType_1.TokenType.null:
            parsed_1 = this.parseNullLiteral();
            break;
        case tokenType_1.TokenType.true:
            parsed_1 = this.parseTrueLiteral();
            break;
        case tokenType_1.TokenType.false:
            parsed_1 = this.parseFalseLiteral();
            break;
        case tokenType_1.TokenType.this:
            parsed_1 = this.parseThisLiteral();
            break;
        case tokenType_1.TokenType.super:
            parsed_1 = this.parseSuperLiteral();
            break;
        case tokenType_1.TokenType.plusPlus:
            parsed_1 = this.parseIncrementExpression();
            break;
        case tokenType_1.TokenType.minusMinus:
            parsed_1 = this.parseDecrementExpression();
            break;
        case tokenType_1.TokenType.equalsGreaterThan:
            parsed_1 = this.parseArrowFunction();
            break;
        case tokenType_1.TokenType.await:
            parsed_1 = this.parseAwaitExpression();
            break;
        default:
            // +Expr
            if (tokenType_1.isUnaryOperator(type_1)) {
                parsed_1 = this.parseUnaryExpression();
                break;
            }
            switch (type_1) {
                ...
                case tokenType_1.TokenType.closeBrace:
                    ...
                    break;
                default:
                    if (tokenType_1.isStatementStart(type_1)) {
                        this.error(this.lexer.peek(), "“{0}”是语句关键字；改用其它变量名?", tokenType_1.tokenToString(type_1));
                    }
                    else {
                        this.error(this.lexer.peek(), "无效的表达式项“{0}”", tokenType_1.tokenToString(type_1));
                    }
                    do {
                        this.lexer.read();
    ...
    return this.parseRestExpression(parsed_1, minPrecedence);
}
parseRestExpression(parsed, nodes.Expression, minPrecedence ?  : number);
{
...
parseParenthesizedExpressionOrArrowFunction();
{
...
parseNewExpression();
{
    ...
    var result_11 = new nodes.NewExpression();
    result_11.start = this.lexer.read().start; // new
    result_11.target = this.parseExpression(0);
    if (this.lexer.peek().type == tokenType_1.TokenType.openParen) {
        result_11.arguments = this.parseArgumentList();
    }
    return result_11;
}
    ...
    while (true) {
        switch (this.lexer.markRead().type) {
            case tokenType_1.TokenType.gt:
                // 如果紧跟 . 说明这是实体泛型。
                return this.lexer.markRead().type == tokenType_1.TokenType.period;
    ...
    while (this.lexer.peek().type.isModifier()) {
        ...
        switch (this.lexer.read().type) {
            case tokenType_1.TokenType.static:
                ...
                ;
                break;
            case tokenType_1.TokenType.virtual:
                current = Modifiers.;
                ;
                break;
            case tokenType_1.TokenType.override:
                current = Modifiers.;
                ;
                break;
            case tokenType_1.TokenType.abstract:
            ...
            case tokenType_1.TokenType.final:
                ...
                break;
            case tokenType_1.TokenType.extern:
                current = Modifiers.;
                ;
                break;
            case tokenType_1.TokenType.volatile:
                current = Modifiers.;
                ;
                break;
            default:
                ...
                do {
                    ...
                    switch (this.lexer.peek().type) {
                        case tokenType_1.TokenType.class:
                            ...
                            ;
                            break;
                        case tokenType_1.TokenType.struct:
...
parseFuncDefinition(DocComment, docComment, MemberDefinition.MemberAnnotation, annotations, Modifiers, modifiers);
{
    ...
    last = null;
    do {
        if (this.lexer.peek().type == tokenType_1.TokenType.rParam) {
            break;
        }
    ...
    while (true) {
        switch (this.lexer.peek().type) {
            case tokenType_1.TokenType.rBrace:
                target.endLocation = this.lexer.read().endLocation;
                return;
            case tokenType_1.TokenType.eof:
                expectToken(tokenType_1.TokenType.rBrace, ErrorCode.expectedRBrace);
                target.endLocation = this.lexer.current.endLocation;
                return;
            default:
...
parseVariableOrExpression();
{
    switch (this.lexer.peek().type) {
        ...
        default:
            if (this.lexer.peek().type.isPredefinedType()) {
...
parseEmbeddedStatement();
{
    ...
    if (result)
        is;
    Semicolon && this.lexer.peek().type == tokenType_1.TokenType.lBrace;
    {
...
parseBlock();
{
    // Block :
    //   { StatementList? }
    console.assert(this.lexer.peek().type == tokenType_1.TokenType.lBrace);
...
parseThrowStatement();
{
    ...
    result.start = this.lexer.read().start; // throw
    if (followsWithExpression()) {
...
parseYieldStatement();
{
    // YieldStatement :
    //   yield Expression ;
    console.assert(this.lexer.peek().type == tokenType_1.TokenType.yield);
    YieldStatement;
...
parseGotoStatement();
{
    // GotoStatement :
    //   goto Identifier ;
    console.assert(this.lexer.peek().type == tokenType_1.TokenType.goto);
    var start = this.lexer.read().start; // goto
    Statement;
    result;
    switch (this.lexer.peek().type) {
        case tokenType_1.TokenType.identifier:
        ...
        case tokenType_1.TokenType.case:
            ...
            {
                target = readToken(tokenType_1.TokenType.else) ? null : parseExpression();
            }
            ;
            break;
        default:
...
parseBreakStatement();
{
    ...
    result.start = this.lexer.read().start; // break
    expectSemicolon();
    result.endLocation = this.lexer.current.endLocation;
    return result;
}
...
parseContinueStatement();
{
    ...
    result.start = this.lexer.read().start; // continue
    expectSemicolon();
    result.endLocation = this.lexer.current.endLocation;
    return result;
}
...
parseTryStatement();
{
    ...
    result.start = this.lexer.read().start; // try
    result.tryClause = parseEmbeddedStatement();
    TryStatement.CatchClause;
    last = null;
    while (readToken(tokenType_1.TokenType.catch)) {
        var current = new TryStatement.CatchClause();
        current.start = this.lexer.current.start;
    ...
    if (readToken(tokenType_1.TokenType.finally)) {
...
parseMagicVariable();
{
    // MagicVariable :
    //   @ Identifier
    console.assert(this.lexer.peek().type == tokenType_1.TokenType.at);
...
parseListOrDictLiteral(tokenType_1.TokenType, stopBrack, ErrorCode, errorCode);
{
    ...
    console.assert(this.lexer.peek().type == (stopBrack == tokenType_1.TokenType.rBrace ? tokenType_1.TokenType.lBrace : tokenType_1.TokenType.lBrack));
...
parseArgumentList(tokenType_1.TokenType, stopBrack, ErrorCode, errorCode);
{
    ...
    //   ref ToExpression
    //   out ToExpression
    console.assert(this.lexer.peek().type == (stopBrack == tokenType_1.TokenType.rParam ? tokenType_1.TokenType.lParam : tokenType_1.TokenType.lBrack));
    this.lexer.read(); // [, (
    FuncCallExpression.Argument;
    first = null, last = null;
    do {
...
{
    if (readToken(tokenType_1.TokenType.out)) {
    ...
    else if (readToken(tokenType_1.TokenType.ref)) {
...
parseLambdaLiteral(Identifier, parsedParameter);
{
    ...
    //    MethodBody
    //    Expression
    console.assert(this.lexer.peek().type == tokenType_1.TokenType.lParam || this.lexer.peek().type == tokenType_1.TokenType.lambda);
        ...
        switch (this.lexer.peek().type) {
            case tokenType_1.TokenType.lBrack:
                ...
                if (typeUsage != TypeUsage.type) {
                    ...
                    do {
                        this.lexer.markRead();
                    } while (this.lexer.markPeek().type == tokenType_1.TokenType.comma);
                    if (this.lexer.markPeek().type != tokenType_1.TokenType.rBrack) {
                        goto;
                    }
                }
            default: ;
...
{
    if (this.lexer.peek().type == tokenType_1.TokenType.lt) {
        // 判断 < 是小于号还是泛型参数。
        if (typeUsage != TypeUsage.type) {
            this.lexer.mark();
        ...
        do {
            if (this.lexer.peek().type == tokenType_1.TokenType.comma || this.lexer.peek().type == tokenType_1.TokenType.gt) {
...
{
    console.assert(this.lexer.markPeek().type == tokenType_1.TokenType.lt);
    do {
        this.lexer.markRead(); // <, ,
        // 允许直接结束。
        if (this.lexer.markPeek().type == tokenType_1.TokenType.gt) {
            break;
        }
        // 如果紧跟的不是类型，则不是类型。
        if (!markReadType()) {
    ...
    // 如果是 > 说明一切顺利。
    return this.lexer.markRead().type == tokenType_1.TokenType.gt;
}
/**
 * 判断一个类型之后是否是数组类型。
 */
...
{
    console.assert(this.lexer.markPeek().type == tokenType_1.TokenType.lBrack);
    this.lexer.markRead(); // [
    // 跳过逗号。
    while (this.lexer.markPeek().type == tokenType_1.TokenType.comma) {
        this.lexer.markRead();
    }
    return this.lexer.markRead().type == tokenType_1.TokenType.rBrack;
}
bool;
markReadType();
{
    ...
    if (type == tokenType_1.TokenType.identifier) {
        if (this.lexer.markPeek().type == tokenType_1.TokenType.lt && !markReadGenericTypeExpression()) {
            return false;
        }
    }
    ...
    while (true) {
        switch (this.lexer.markPeek().type) {
            ...
            case tokenType_1.TokenType.period:
                this.lexer.markRead();
                if (this.lexer.markRead().type != tokenType_1.TokenType.identifier) {
                    return false;
                }
                continue;
            default:
...
(function (IncrementalParser) {
    ...
    SourceFile;
    {
        ...
        // are already correct.
        var result_13 = Parser.parseSourceFile(sourceFile.fileName, newText, sourceFile.languageVersion, syntaxCursor, /*setParentNodes*/ true, sourceFile.scriptKind);
        return result_13;
    }
