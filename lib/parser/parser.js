...
var Parser = (function () {
     ...
     */
    Parser.prototype.parseFunctionOrConstructorTypeNode = function (constructor) {
        var result = constructor ? new nodes.ConstructorTypeNode() : new nodes.FunctionTypeNode();
        if (constructor)
    ...
    Parser.prototype.parseUnionOrIntersectionTypeOrHigher = function (type) {
        ...
        while (this.lexer.peek().type === type) {
            var unionOrIntersectionType = type === tokenType_1.TokenType.ampersand ? new nodes.IntersectionTypeNode() : new nodes.UnionTypeNode();
            unionOrIntersectionType.leftOperand = result;
            unionOrIntersectionType.operatorToken = this.readToken(type);
            unionOrIntersectionType.rightOperand = type === tokenType_1.TokenType.ampersand ? this.parseArrayTypeOrHigher() : this.parseUnionOrIntersectionTypeOrHigher(tokenType_1.TokenType.ampersand);
