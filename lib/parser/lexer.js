...
var Lexer = (function () {
    ...
    Lexer.prototype.skipShebang = function () {
        if (this.source.charCodeAt(this.pos) === unicode_1.CharCode.hash && this.source.charCodeAt(this.pos + 1) === unicode_1.CharCode.exclamation) {
            this.pos += 2;
            this.skipLine();
        }
    };
    ...
    Lexer.prototype.scan = function () {
        ...
        while (true) {
            ...
            // 标识符、关键字
            if (ch >= unicode_1.CharCode.a && ch <= unicode_1.CharCode.z) {
            ...
            switch (ch) {
                // \s、\t
                case unicode_1.CharCode.space:
                case unicode_1.CharCode.horizontalTab:
                    // 加速连续空格解析。
                    ...
                    continue;
                // \r、\n
                case unicode_1.CharCode.carriageReturn:
                case unicode_1.CharCode.lineFeed:
                    result.hasLineBreakBeforeStart = true;
                    continue;
                // /、//、/*、/=
                case unicode_1.CharCode.slash:
                    switch (this.source.charCodeAt(this.pos++)) {
                        case unicode_1.CharCode.slash:
                            ...
                            continue;
                        case unicode_1.CharCode.asterisk:
                            ...
                            while (this.pos < this.source.length) {
                                ...
                                else if (ch === unicode_1.CharCode.asterisk && this.source.charCodeAt(this.pos + 1) === unicode_1.CharCode.slash) {
                            ...
                            continue;
                        case unicode_1.CharCode.equals:
                    ...
                    break;
                // .1、..、...
                case unicode_1.CharCode.dot:
                    if (Unicode.isDecimalDigit(this.pos)) {
                        this.pos--;
                        result.data = this.scanNumericLiteral(unicode_1.CharCode.num0);
                        result.type = tokenType_1.TokenType.numericLiteral;
                        break;
                    }
                    if (this.source.charCodeAt(this.pos) === unicode_1.CharCode.dot) {
                        this.pos++; // .
                        if (this.source.charCodeAt(this.pos) === unicode_1.CharCode.dot) {
                            this.pos++; // .
                ...
                case unicode_1.CharCode.equals:
                    switch (this.source.charCodeAt(this.pos)) {
                        case unicode_1.CharCode.equals:
                            this.pos++; // =
                            if (this.source.charCodeAt(this.pos) === unicode_1.CharCode.equals) {
                                this.pos++; // =
                                if (compiler_1.options.allowGitConflictMarker !== false && result.hasLineBreakBeforeStart && this.skipGitConflictMarker(unicode_1.CharCode.equals, 4)) {
                                    // 跳过冲突的第二个版本。
                                    while (this.pos < this.source.length) {
                                        if (this.skipGitConflictMarker(unicode_1.CharCode.greaterThan, 7)) {
                                            break;
                                        }
                                        this.skipLine();
                                    }
                        ...
                        case unicode_1.CharCode.greaterThan:
                            this.pos++; // >
                    ...
                    break;
                // '、"
                case unicode_1.CharCode.singleQuote:
                case unicode_1.CharCode.doubleQuote:
                    result.data = this.scanStringLiteral(ch);
                    result.type = tokenType_1.TokenType.stringLiteral;
                    break;
                // `
                case unicode_1.CharCode.backtick:
                    result.data = this.scanStringLiteral(ch);
                    result.type = this.source.charCodeAt(this.pos - 1) === unicode_1.CharCode.openBrace ? tokenType_1.TokenType.templateHead : tokenType_1.TokenType.noSubstitutionTemplateLiteral;
                    break;
                // +、++、+=
                case unicode_1.CharCode.plus:
                    switch (this.source.charCodeAt(this.pos)) {
                        case unicode_1.CharCode.plus:
                            this.pos++; // +
                    ...
                    switch (this.source.charCodeAt(this.pos++)) {
                        ...
                        default:
                            ...
                            result.data = Unicode.isOctalDigit(this.source.charCodeAt(--this.pos)) ?
                                this.scanDights(8) :
                                this.scanNumericLiteral(unicode_1.CharCode.num0);
                            break;
                    }
                    result.type = tokenType_1.TokenType.numericLiteral;
                    break;
                // @
                case unicode_1.CharCode.at:
                    result.type = tokenType_1.TokenType.at;
                    break;
                // !、!=、!==
                case unicode_1.CharCode.exclamation:
                    if (this.source.charCodeAt(this.pos) === unicode_1.CharCode.equals) {
                        if (this.source.charCodeAt(++this.pos) === unicode_1.CharCode.equals) {
                            this.pos++; // =
                    ...
                    break;
                // %、%=
                case unicode_1.CharCode.percent:
                    ...
                    break;
                // &、&&、&=
                case unicode_1.CharCode.ampersand:
                    switch (this.source.charCodeAt(this.pos)) {
                        case unicode_1.CharCode.ampersand:
                            this.pos++; // &
                ...
                case unicode_1.CharCode.asterisk:
                    ...
                    break;
                // <、<<、<<=、<=
                case unicode_1.CharCode.lessThan:
                    switch (this.source.charCodeAt(this.pos)) {
                        case unicode_1.CharCode.lessThan:
                            this.pos++; // <
                            if (compiler_1.options.allowGitConflictMarker !== false && result.hasLineBreakBeforeStart && this.skipGitConflictMarker(unicode_1.CharCode.lessThan, 5)) {
                                continue;
                            }
                            if (this.source.charCodeAt(this.pos) === unicode_1.CharCode.equals) {
                                this.pos++; // =
                ...
                case unicode_1.CharCode.bar:
                    ...
                    break;
                // ~
                case unicode_1.CharCode.tilde:
                    result.type = tokenType_1.TokenType.tilde;
                    break;
                // ^、^=
                case unicode_1.CharCode.caret:
                    if (this.source.charCodeAt(this.pos) === unicode_1.CharCode.equals) {
                        this.pos++; // =
                    ...
                    break;
                // \u0000
                case unicode_1.CharCode.backslash:
                    if (this.source.charCodeAt(this.pos) === unicode_1.CharCode.u) {
                        this.pos++;
                        ch = this.scanDights(16, 4);
                        if (Unicode.isIdentifierStart(ch)) {
                            result.data = this.scanIdentifier(ch);
                            if ((compiler_1.options.languageVersion === LanguageVersion.javaScript3 || compiler_1.options.languageVersion === LanguageVersion.javaScript) && tokenType_1.stringToToken(result.data) !== tokenType_1.TokenType.identifier) {
    ...
    Lexer.prototype.scanIdentifier = function (currentChar) {
        ...
        while (true) {
            ...
            if (!Unicode.isIdentifierPart(ch)) {
                if (ch === unicode_1.CharCode.backslash) {
                    // 处理反斜杠之前的标识符部分。
                    if (result == undefined) {
                ...
                case unicode_1.CharCode.backslash:
                    ...
                    switch (ch) {
                        ...
                        case unicode_1.CharCode.backtick:
                            result += '`';
                            break;
                        case unicode_1.CharCode.n:
                            result += '\n';
                            break;
                        case unicode_1.CharCode.r:
                            result += '\r';
                            break;
                        case unicode_1.CharCode.t:
                            result += '\t';
                            break;
                        case unicode_1.CharCode.u:
                            // \u{00000000}
                            if (compiler_1.options.languageVersion !== LanguageVersion.javaScript3 &&
                                this.source.charCodeAt(this.pos) === unicode_1.CharCode.openBrace) {
                                this.pos++; // {
                                ...
                                if (this.source.charCodeAt(this.pos++) !== unicode_1.CharCode.closeBrace) {
                                    this.pos--;
                        ...
                        case unicode_1.CharCode.b:
                            result += '\b';
                            break;
                        case unicode_1.CharCode.v:
                            result += '\v';
                            break;
                        case unicode_1.CharCode.f:
                            result += '\f';
                            break;
                        case unicode_1.CharCode.carriageReturn:
                            if (this.source.charCodeAt(this.pos) === unicode_1.CharCode.lineFeed) {
                                this.pos++; // \n
                            }
                        // 继续执行。
                        case unicode_1.CharCode.lineFeed:
                        case unicode_1.CharCode.lineSeparator:
                        case unicode_1.CharCode.paragraphSeparator:
                            break;
                        case undefined:
                            this.pos--;
                            this.error("应输入转义字符。");
                    ...
                    start = this.pos;
                    continue;
                case unicode_1.CharCode.dollar:
                    this.pos++; // $
                    // 模板字符串中的 ${ 。
                    if (currentChar === unicode_1.CharCode.backtick &&
                        this.source.charCodeAt(this.pos) === unicode_1.CharCode.openBrace) {
    ...
    Lexer.prototype.scanNumericLiteral = function (currentChar) {
        // 读取整数部分。
        var result = currentChar - unicode_1.CharCode.num0;
        while (true) {
            var num = this.source.charCodeAt(this.pos) - unicode_1.CharCode.num0;
            if (num >= 0 && num <= 9) {
                this.pos++; // 整数部分。
    ...
    Lexer.prototype.scanDights = function (base, count, max) {
        ...
        while (true) {
            ...
            num = base <= 9 || num >= unicode_1.CharCode.num0 && num <= unicode_1.CharCode.num9 ? num - unicode_1.CharCode.num0 :
                num >= unicode_1.CharCode.A && num <= unicode_1.CharCode.Z ? 10 + num - unicode_1.CharCode.A :
                    num >= unicode_1.CharCode.a && num <= unicode_1.CharCode.z ? 10 + num - unicode_1.CharCode.a : -1;
            // 解析到不合法的数字或超过范围则停止解析。
            if (num < 0 || num >= base ||
    ...
    Lexer.prototype.skipGitConflictMarker = function (currentChar, repeatCount) {
        ...
        if (this.source.charCodeAt(this.pos + repeatCount) !== unicode_1.CharCode.space) {
            return false;
        }
    ...
    Lexer.prototype.readAsRegularExpressionLiteral = function () {
        console.assert(this.source.charCodeAt(this.current.start) === unicode_1.CharCode.slash);
        // FIXME: 需要测试正则表达式语法?
        var pattern;
        var flags;
        var data;
    ...
    Lexer.prototype.readAsTemplateMiddleOrTail = function () {
        console.assert(this.source.charCodeAt(this.current.start) === unicode_1.CharCode.closeBrace);
        var start = this.current.start + 1;
        this.pos = start;
        var data = this.scanStringLiteral(unicode_1.CharCode.backtick);
        return this.current = this.current._next = {
            start: this.current.start,
            type: this.source.charCodeAt(this.pos - 1) === unicode_1.CharCode.openBrace ? tokenType_1.TokenType.templateMiddle : tokenType_1.TokenType.templateTail,
            data: data,
            end: this.pos
        };
    };
