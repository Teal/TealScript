...
var Lexer = (function () {
    ...
    Lexer.prototype.setSource = function (text, start, fileName) {
        ...
        // 跳过开头的 #! 部分。
        if (!compiler_1.options.disallowShebang) {
            this.skipShebang();
        }
        // 预读第一个标记。
            ...
            switch (ch) {
                    ...
                    switch (this.source.charCodeAt(this.pos)) {
                        case charCode_1.CharCode.equals:
                            ...
                            if (this.source.charCodeAt(this.pos) === charCode_1.CharCode.equals) {
                                this.pos++; // =
                                if (compiler_1.options.allowGitConflictMarker !== false && result.hasLineBreakBeforeStart && this.skipGitConflictMarker(charCode_1.CharCode.equals, 4)) {
                                    // 跳过冲突的第二个版本。
                                    while (this.pos < this.source.length) {
                ...
                case charCode_1.CharCode.lessThan:
                    switch (this.source.charCodeAt(this.pos)) {
                        case charCode_1.CharCode.lessThan:
                            this.pos++; // <
                            if (compiler_1.options.allowGitConflictMarker !== false && result.hasLineBreakBeforeStart && this.skipGitConflictMarker(charCode_1.CharCode.lessThan, 5)) {
                                continue;
                            }
                            if (this.source.charCodeAt(this.pos) === charCode_1.CharCode.equals) {
                                this.pos++; // =
