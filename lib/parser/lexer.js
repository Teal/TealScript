...
var Lexer = (function () {
     ...
     */
    Lexer.prototype.setSource = function (text, start) {
        if (start === void 0) { start = 0; }
        this.source = text;
        this.pos = start;
        delete this.comments;
        // 跳过开头的 #! 部分。
        if (compiler_1.options.skipShebang !== false) {
            this.skipShebang();
        }
        // 预读第一个标记。
        ...
        while (true) {
            ...
            switch (ch) {
                ...
                case charCode_1.CharCode.slash:
                    switch (this.source.charCodeAt(this.pos)) {
                        ...
                        case charCode_1.CharCode.asterisk:
                            ...
                            if ((compiler_1.options.parseComments & compiler_1.ParseCommentsOption.jsDoc) ? this.source.charCodeAt(multiCommentStart) === charCode_1.CharCode.asterisk : (compiler_1.options.parseComments & compiler_1.ParseCommentsOption.multiLine)) {
                ...
                case charCode_1.CharCode.exclamation:
                    if (this.pos < this.sourceEnd && this.source.charCodeAt(this.pos) === charCode_1.CharCode.equals) {
                        this.pos++;
                        if (this.pos < this.sourceEnd && this.source.charCodeAt(this.pos) === charCode_1.CharCode.equals) {
                            this.pos++;
                ...
                case charCode_1.CharCode.percent:
                    if (this.pos < this.sourceEnd && this.source.charCodeAt(this.pos) === tokenType_1.TokenType.equals) {
                        this.pos++;
                ...
                case charCode_1.CharCode.caret:
                    if (this.source.charCodeAt(this.pos) === tokenType_1.TokenType.caretEquals) {
                        this.pos++;
        ...
        while (this.pos < this.sourceEnd) {
            ...
            if (!Unicode.isIdentifierPart(ch)) {
                if (ch === charCode_1.CharCode.backslash) {
                    ...
                    // 处理转义字符。
                    if (++this.pos < this.sourceEnd &&
                        this.source.charCodeAt(this.pos) === charCode_1.CharCode.u) {
                        this.pos++;
    ...
    Lexer.prototype.scanStringLiteral = function (currentChar) {
        ...
        if (compiler_1.options.languageVersion === compiler_1.LanguageVersion.tealScript &&
            ...
            if (compiler_1.options.skipUnterminatedLiteral === false) {
                    ...
                    switch (ch) {
                        ...
                        case charCode_1.CharCode.u:
                            ...
                            if (compiler_1.options.languageVersion !== compiler_1.LanguageVersion.javaScript3 &&
                                ...
                                if (this.pos >= this.sourceEnd || this.source.charCodeAt(this.pos) !== charCode_1.CharCode.closeBrace) {
    ...
    Lexer.prototype.scanNumericLiteral = function (currentChar) {
        // 0x00, 0O00, 0b00
        if (currentChar === charCode_1.CharCode.num0 && this.pos < this.sourceEnd) {
            switch (this.source.charCodeAt(this.pos++)) {
                case charCode_1.CharCode.x:
                case charCode_1.CharCode.X:
     ...
     * @return 返回解析的数值。
     */
    Lexer.prototype.scanDights = function (base, count) {
        var result = 0;
        var start = this.pos;
        var end = count == undefined ? this.sourceEnd : this.pos + count;
        while (this.pos < this.sourceEnd) {
    ...
    // #endregion
    // #region 延时解析
    Lexer.prototype.readAsRegExpLiteral = function (currentChar) {
    };
    return Lexer;
}());
exports.Lexer = Lexer;
