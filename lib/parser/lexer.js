...
var Lexer = (function () {
    ...
    Lexer.prototype.error = function (message) {
        ...
        compiler_1.error.apply(void 0, [compiler_1.ErrorType.lexicalError, message].concat(args));
    };
    /**
     * 跳过开头的 #! 标记。
     */
            ...
            switch (ch) {
                    ...
                    switch (this.source.charCodeAt(this.pos)) {
                        case charCode_1.CharCode.equals:
                            ...
                            if (this.source.charCodeAt(this.pos) === charCode_1.CharCode.equals) {
                                this.pos++; // =
                                if (compiler_1.options.skipGitConflictMarker !== false && result.onNewLine && this.skipGitConflictMarker(charCode_1.CharCode.equals, 4)) {
                                    // 跳过冲突的第二个版本。
                                    while (this.pos < this.source.length) {
                ...
                case charCode_1.CharCode.lessThan:
                    switch (this.source.charCodeAt(this.pos)) {
                        case charCode_1.CharCode.lessThan:
                            this.pos++; // <
                            if (compiler_1.options.skipGitConflictMarker !== false && result.onNewLine && this.skipGitConflictMarker(charCode_1.CharCode.lessThan, 5)) {
                                continue;
                            }
                            if (this.source.charCodeAt(this.pos) === charCode_1.CharCode.equals) {
                                this.pos++; // =
     ...
     * 以正则表达式重新读取下一个标记。
     */
    Lexer.prototype.readAsRegExpLiteral = function () {
