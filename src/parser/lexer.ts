/**
 * @fileOverview 词法分析器
 * @decription 词法分析器负责将源码解析为多个标记的序列。
 * @author xuld@vip.qq.com
 */
import {options, error, ErrorType} from '../compiler/compiler';
import {TokenType, stringToToken} from './tokenType';
...
export class Lexer {
     ...
     */
    setSource(text: string, start = 0, options?: LexerOptions) {
        this.source = text;
        this.pos = start;
            ...
            switch (ch) {
                    ...
                    switch (this.source.charCodeAt(this.pos)) {
                        case CharCode.equals:
                            ...
                            if (this.source.charCodeAt(this.pos) === CharCode.equals) {
                                this.pos++; // =
                                if (options.allowGitConflictMarker !== false && result.hasLineBreakBeforeStart && this.skipGitConflictMarker(CharCode.equals, 4)) {
                                    // 跳过冲突的第二个版本。
                                    while (this.pos < this.source.length) {
                ...
                case CharCode.lessThan:
                    switch (this.source.charCodeAt(this.pos)) {
                        case CharCode.lessThan:
                            this.pos++; // <
                            if (options.allowGitConflictMarker !== false && result.hasLineBreakBeforeStart && this.skipGitConflictMarker(CharCode.lessThan, 5)) {
                                continue;
                            }
                            if (this.source.charCodeAt(this.pos) === CharCode.equals) {
                                this.pos++; // =
...
export interface LexerOptions {
     ...
     * @param args 格式化错误的参数。
     */
    error?: (start: number, end: number, message: string, ...args: any[]) => void;
