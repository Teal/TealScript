...
namespace ts {
    ...
    export function getRootLength(path: string): number {
        if (path.charCodeAt(0) === CharCode.slash) {
            if (path.charCodeAt(1) !== CharCode.slash) return 1;
        ...
        if (path.charCodeAt(1) === CharCode.colon) {
            if (path.charCodeAt(2) === CharCode.slash) return 3;
            return 2;
        }
    ...
    function getNormalizedPathComponentsOfUrl(url: string) {
        ...
        while (rootLength < urlLength) {
            ...
            if (url.charCodeAt(rootLength) === CharCode.slash) {
                rootLength++;
            }
            else {
    ...
    const wildcardCharCodes = [CharCode.asterisk, CharCode.question];
    ...
    export interface ObjectAllocator {
        getNodeConstructor(): new (kind: TokenType, pos?: number, end?: number) => Node;
        getSourceFileConstructor(): new (kind: TokenType, pos?: number, end?: number) => SourceFile;
    ...
    function Node(kind: TokenType, pos: number, end: number) {
        this.kind = kind;
        this.pos = pos;
        this.end = end;
