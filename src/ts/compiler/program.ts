...
namespace ts {
    ...
    export function hasZeroOrOneAsteriskCharacter(str: string): boolean {
        ...
        for (let i = 0; i < str.length; i++) {
            if (str.charCodeAt(i) === CharCode.asterisk) {
                if (!seenAsterisk) {
                    seenAsterisk = true;
                }
                else {
    ...
    function moduleHasNonRelativeName(moduleName: string): boolean {
        ...
        const startsWithDotSlashOrDotDotSlash = i === 0 || (i === 1 && moduleName.charCodeAt(0) === CharCode.dot);
            ...
            function collectModuleReferences(node: Node, inAmbientModule: boolean): void {
                switch (node.kind) {
                    ...
                    case TokenType.ExportDeclaration:
                        ...
                        if (!moduleNameExpr || moduleNameExpr.kind !== TokenType.StringLiteral) {
                            break;
                        }
