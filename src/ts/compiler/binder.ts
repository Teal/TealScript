...
namespace ts {
    ...
    export function getModuleInstanceState(node: Node): ModuleInstanceState {
        ...
        if (node.kind === TokenType.InterfaceDeclaration || node.kind === TokenType.TypeAliasDeclaration) {
        ...
        else if ((node.kind === TokenType.ImportDeclaration || node.kind === TokenType.ImportEqualsDeclaration) && !(node.flags & NodeFlags.Export)) {
            return ModuleInstanceState.NonInstantiated;
        }
        // 4. other uninstantiated module declarations.
        else if (node.kind === TokenType.ModuleBlock) {
        ...
        else if (node.kind === TokenType.ModuleDeclaration) {
    ...
    function createBinder(): (file: SourceFile, options: CompilerOptions) => void {
        ...
        function addDeclarationToSymbol(symbol: Symbol, node: Declaration, symbolFlags: SymbolFlags) {
            ...
            if (symbolFlags & SymbolFlags.Value) {
                ...
                if (!valueDeclaration ||
                    (valueDeclaration.kind !== node.kind && valueDeclaration.kind === TokenType.ModuleDeclaration)) {
        ...
        function getDeclarationName(node: Declaration): string {
            if (node.name) {
                ...
                if (node.name.kind === TokenType.ComputedPropertyName) {
            ...
            switch (node.kind) {
                ...
                case TokenType.Parameter:
                    ...
                    Debug.assert(node.parent.kind === TokenType.JSDocFunctionType);
                ...
                case TokenType.JSDocTypedefTag:
                    ...
                    if (parentNode && parentNode.kind === TokenType.VariableStatement) {
                        if ((<VariableStatement>parentNode).declarationList.declarations.length > 0) {
                            const nameIdentifier = (<VariableStatement>parentNode).declarationList.declarations[0].name;
                            if (nameIdentifier.kind === TokenType.Identifier) {
        ...
        function declareModuleMember(node: Declaration, symbolFlags: SymbolFlags, symbolExcludes: SymbolFlags): Symbol {
            ...
            if (symbolFlags & SymbolFlags.Alias) {
                if (node.kind === TokenType.ExportSpecifier || (node.kind === TokenType.ImportEqualsDeclaration && hasExportModifier)) {
        ...
        function bindContainer(node: Node, containerFlags: ContainerFlags) {
            ...
            if (containerFlags & ContainerFlags.IsControlFlowContainer) {
                ...
                if (node.kind === TokenType.SourceFile) {
        ...
        function hasNarrowableArgument(expr: CallExpression) {
            ...
            if (expr.expression.kind === TokenType.PropertyAccessExpression &&
        ...
        function isNarrowingTypeofOperands(expr1: Expression, expr2: Expression) {
            return expr1.kind === TokenType.TypeOfExpression && isNarrowableOperand((<TypeOfExpression>expr1).expression) && expr2.kind === TokenType.StringLiteral;
        }

        function isNarrowingDiscriminant(expr: Expression) {
            return expr.kind === TokenType.PropertyAccessExpression && isNarrowableReference((<PropertyAccessExpression>expr).expression);
        }
        ...
        function bindReturnOrThrow(node: ReturnStatement | ThrowStatement): void {
            bind(node.expression);
            if (node.kind === TokenType.ReturnStatement) {
        ...
        function bindbreakOrContinueFlow(node: BreakOrContinueStatement, breakTarget: FlowLabel, continueTarget: FlowLabel) {
            const flowLabel = node.kind === TokenType.BreakStatement ? breakTarget : continueTarget;
            if (flowLabel) {
        ...
        function bindSwitchStatement(node: SwitchStatement): void {
            ...
            const hasDefault = forEach(node.caseBlock.clauses, c => c.kind === TokenType.DefaultClause);
        ...
        function bindAssignmentTargetFlow(node: Expression) {
            ...
            else if (node.kind === TokenType.ArrayLiteralExpression) {
            ...
            else if (node.kind === TokenType.ObjectLiteralExpression) {
                for (const p of (<ObjectLiteralExpression>node).properties) {
                    if (p.kind === TokenType.PropertyAssignment) {
                        bindDestructuringTargetFlow((<PropertyAssignment>p).initializer);
                    }
                    else if (p.kind === TokenType.ShorthandPropertyAssignment) {
        ...
        function bindDeleteExpressionFlow(node: DeleteExpression) {
            forEachChild(node, bind);
            if (node.expression.kind === TokenType.PropertyAccessExpression) {
        ...
        function bindVariableDeclarationFlow(node: VariableDeclaration) {
            forEachChild(node, bind);
            if (node.initializer || node.parent.parent.kind === TokenType.ForInStatement || node.parent.parent.kind === TokenType.ForOfStatement) {
        ...
        function bindCallExpressionFlow(node: CallExpression) {
            ...
            while (expr.kind === TokenType.ParenthesizedExpression) {
                expr = (<ParenthesizedExpression>expr).expression;
            }
            if (expr.kind === TokenType.FunctionExpression || expr.kind === TokenType.ArrowFunction) {
        ...
        function bindModuleDeclaration(node: ModuleDeclaration) {
            ...
            if (isAmbientModule(node)) {
                ...
                if (isExternalModuleAugmentation(node)) {
                    ...
                    let pattern: Pattern;
                    if (node.name.kind === TokenType.StringLiteral) {
        ...
        function bindObjectLiteralExpression(node: ObjectLiteralExpression) {
            ...
            if (inStrictMode) {
                ...
                for (const prop of node.properties) {
                    if (prop.name.kind !== TokenType.Identifier) {
                        continue;
                    }
                    ...
                    const currentKind = prop.kind === TokenType.PropertyAssignment || prop.kind === TokenType.ShorthandPropertyAssignment || prop.kind === TokenType.MethodDeclaration
        ...
        function checkStrictModeIdentifier(node: Identifier) {
            if (inStrictMode &&
                node.originalKeywordKind >= TokenType.FirstFutureReservedWord &&
                node.originalKeywordKind <= TokenType.LastFutureReservedWord &&
        ...
        function checkStrictModeDeleteExpression(node: DeleteExpression) {
            // Grammar checking
            if (inStrictMode && node.expression.kind === TokenType.Identifier) {
        ...
        function isEvalOrArgumentsIdentifier(node: Node): boolean {
            return node.kind === TokenType.Identifier &&
                ((<Identifier>node).text === "eval" || (<Identifier>node).text === "arguments");
        }

        function checkStrictModeEvalOrArguments(contextNode: Node, name: Node) {
            if (name && name.kind === TokenType.Identifier) {
        ...
        function checkStrictModeFunctionDeclaration(node: FunctionDeclaration) {
            if (languageVersion < ScriptTarget.ES6) {
                ...
                if (blockScopeContainer.kind !== TokenType.SourceFile &&
                    blockScopeContainer.kind !== TokenType.ModuleDeclaration &&
        ...
        function checkTypePredicate(node: TypePredicateNode) {
            ...
            if (parameterName && parameterName.kind === TokenType.Identifier) {
                checkStrictModeIdentifier(parameterName as Identifier);
            }
            if (parameterName && parameterName.kind === TokenType.ThisType) {
                seenThisKeyword = true;
            }
            bind(type);
        }
        ...
        function bindExportAssignment(node: ExportAssignment | BinaryExpression) {
            const boundExpression = node.kind === TokenType.ExportAssignment ? (<ExportAssignment>node).expression : (<BinaryExpression>node).right;
            if (!container.symbol || !container.symbol.exports) {
                // Export assignment in some sort of block construct
                bindAnonymousDeclaration(node, SymbolFlags.Alias, getDeclarationName(node));
            }
            else if (boundExpression.kind === TokenType.Identifier && node.kind === TokenType.ExportAssignment) {
        ...
        function bindThisPropertyAssignment(node: BinaryExpression) {
            ...
            let assignee: Node;
            if (container.kind === TokenType.FunctionDeclaration || container.kind === TokenType.FunctionDeclaration) {
                assignee = container;
            }
            else if (container.kind === TokenType.Constructor) {
        ...
        function checkUnreachable(node: Node): boolean {
            ...
            if (currentFlow === unreachableFlow) {
                const reportError =
                    ...
                    (isStatement(node) && node.kind !== TokenType.EmptyStatement) ||
                    // report error on class declarations
                    node.kind === TokenType.ClassDeclaration ||
                    // report error on instantiated modules or const-enums only modules if preserveConstEnums is set
                    (node.kind === TokenType.ModuleDeclaration && shouldReportErrorOnModuleDeclaration(<ModuleDeclaration>node)) ||
                    // report error on regular enums and const enums if preserveConstEnums is set
                    (node.kind === TokenType.EnumDeclaration && (!isConstEnumDeclaration(node) || options.preserveConstEnums));
                ...
                if (reportError) {
                    ...
                    const reportUnreachableCode =
                        ...
                        !isInAmbientContext(node) &&
                        (
                            node.kind !== TokenType.VariableStatement ||
