    ...
    export function parseCommandLine(commandLine: string[], readFile?: (path: string) => string): ParsedCommandLine {
        ...
        function parseStrings(args: string[]) {
            ...
            while (i < args.length) {
                let s = args[i];
                i++;
                if (s.charCodeAt(0) === CharCode.at) {
                    parseResponseFile(s.slice(1));
                }
                else if (s.charCodeAt(0) === CharCode.minus) {
                    s = s.slice(s.charCodeAt(1) === CharCode.minus ? 2 : 1).toLowerCase();
        ...
        function parseResponseFile(fileName: string) {
            ...
            let pos = 0;
            while (true) {
                while (pos < text.length && text.charCodeAt(pos) <= CharCode.space) pos++;
                if (pos >= text.length) break;
                const start = pos;
                if (text.charCodeAt(start) === CharCode.doubleQuote) {
                    pos++;
                    while (pos < text.length && text.charCodeAt(pos) !== CharCode.doubleQuote) pos++;
                    if (pos < text.length) {
                    ...
                    while (text.charCodeAt(pos) > CharCode.space) pos++;
