        ...
        function createFileEmitter(): (jsFilePath: string, sourceMapFilePath: string, sourceFiles: SourceFile[], isBundledEmit: boolean) => void {
            ...
            function makeTempVariableName(flags: TempFlags): string {
                ...
                while (true) {
                    ...
                    // Skip over 'i' and 'n'
                    if (count !== 8 && count !== 13) {
                        const name = count < 26 ? "_" + String.fromCharCode(CharCode.a + count) : "_" + (count - 26);
                        if (isUniqueName(name)) {
                            return name;
                        }
                    }
            ...
            function makeUniqueName(baseName: string): string {
                ...
                if (baseName.charCodeAt(baseName.length - 1) !== CharCode.underline) {
                    baseName += "_";
                }
                let i = 1;
                while (true) {
            ...
            function generateNameForImportOrExportDeclaration(node: ImportDeclaration | ExportDeclaration) {
                ...
                const baseName = expr.kind === TokenType.StringLiteral ?
            ...
            function createTempVariable(flags: TempFlags): Identifier {
                const result = <Identifier>createSynthesizedNode(TokenType.Identifier);
            ...
            function emitToken(tokenKind: TokenType, startPos: number, emitFn?: () => void) {
            ...
            function isBinaryOrOctalIntegerLiteral(node: LiteralLikeNode, text: string): boolean {
                if (node.kind === TokenType.NumericLiteral && text.length > 1) {
            ...
            function emitLiteral(node: LiteralExpression | TemplateLiteralFragment) {
                ...
                if ((compilerOptions.sourceMap || compilerOptions.inlineSourceMap) && (node.kind === TokenType.StringLiteral || isTemplateLiteralKind(node.kind))) {
            ...
            function emitDownlevelRawTemplateLiteral(node: LiteralExpression) {
                ...
                const isLast = node.kind === TokenType.NoSubstitutionTemplateLiteral || node.kind === TokenType.TemplateTail;
            ...
            function emitDownlevelTaggedTemplateArray(node: TaggedTemplateExpression, literalEmitter: (literal: LiteralExpression | TemplateLiteralFragment) => void) {
                write("[");
                if (node.template.kind === TokenType.NoSubstitutionTemplateLiteral) {
            ...
            function emitDownlevelTaggedTemplate(node: TaggedTemplateExpression) {
                ...
                if (node.template.kind === TokenType.TemplateExpression) {
            ...
            function emitTemplateExpression(node: TemplateExpression): void {
                ...
                for (let i = 0, n = node.templateSpans.length; i < n; i++) {
                    ...
                    // rather than
                    //    "abc" + (1 << 2) + ""
                    const needsParens = templateSpan.expression.kind !== TokenType.ParenthesizedExpression
            ...
            function jsxEmitReact(node: JsxElement | JsxSelfClosingElement) {
                ...
                function emitTagName(name: LeftHandSideExpression) {
                    if (name.kind === TokenType.Identifier && isIntrinsicJsxName((<Identifier>name).text)) {
                        write('"');
                        emit(name);
                        write('"');
                    }
                ...
                function emitJsxElement(openingNode: JsxOpeningLikeElement, children?: JsxChild[]) {
                    const syntheticReactRef = <Identifier>createSynthesizedNode(TokenType.Identifier);
                    ...
                    if (openingNode.attributes.length === 0) {
                        ...
                        if (forEach(attrs, attr => attr.kind === TokenType.JsxSpreadAttribute)) {
                            write("__assign(");

                            let haveOpenedObjectLiteral = false;
                            for (let i = 0; i < attrs.length; i++) {
                                if (attrs[i].kind === TokenType.JsxSpreadAttribute) {
                                    ...
                                    Debug.assert(attrs[i].kind === TokenType.JsxAttribute);
                                    if (haveOpenedObjectLiteral) {
                                        write(", ");
                                    }
                                    else {
                    ...
                    if (children) {
                        ...
                        if (multipleEmittableChildren) {
                            ...
                            if (firstChild.kind !== TokenType.JsxElement && firstChild.kind !== TokenType.JsxSelfClosingElement) {
                                emit(firstChild);
                            }
                            else {
                ...
                if (node.kind === TokenType.JsxElement) {
                    emitJsxElement((<JsxElement>node).openingElement, (<JsxElement>node).children);
                }
                else {
                    Debug.assert(node.kind === TokenType.JsxSelfClosingElement);
            ...
            function jsxEmitPreserve(node: JsxElement | JsxSelfClosingElement) {
                ...
                function emitAttributes(attribs: NodeArray<JsxAttribute | JsxSpreadAttribute>) {
                    for (let i = 0, n = attribs.length; i < n; i++) {
                        ...
                        if (attribs[i].kind === TokenType.JsxSpreadAttribute) {
                            emitJsxSpreadAttribute(<JsxSpreadAttribute>attribs[i]);
                        }
                        else {
                            Debug.assert(attribs[i].kind === TokenType.JsxAttribute);
                ...
                function emitJsxOpeningOrSelfClosingElement(node: JsxOpeningElement | JsxSelfClosingElement) {
                    write("<");
                    emit(node.tagName);
                    if (node.attributes.length > 0 || (node.kind === TokenType.JsxSelfClosingElement)) {
                        write(" ");
                    }

                    emitAttributes(node.attributes);

                    if (node.kind === TokenType.JsxSelfClosingElement) {
                        write("/>");
                    }
                    else {
                        write(">");
            ...
            function emitExpressionForPropertyName(node: DeclarationName) {
                Debug.assert(node.kind !== TokenType.BindingElement);

                if (node.kind === TokenType.StringLiteral) {
                    emitLiteral(<LiteralExpression>node);
                }
                else if (node.kind === TokenType.ComputedPropertyName) {
            ...
            function emitExpressionIdentifier(node: Identifier) {
                ...
                if (container) {
                    if (container.kind === TokenType.SourceFile) {
                    ...
                    if (modulekind !== ModuleKind.ES6) {
                        ...
                        if (declaration) {
                            if (declaration.kind === TokenType.ImportClause) {
                            ...
                            else if (declaration.kind === TokenType.ImportSpecifier) {
            ...
            function getClassExpressionInPropertyAccessInStaticPropertyDeclaration(node: Identifier) {
                if (languageVersion >= ScriptTarget.ES6) {
                    ...
                    if (parent.kind === TokenType.PropertyAccessExpression && (<PropertyAccessExpression>parent).expression === node) {
                        parent = parent.parent;
                        while (parent && parent.kind !== TokenType.PropertyDeclaration) {
                            parent = parent.parent;
                        }
                        return parent && parent.kind === TokenType.PropertyDeclaration && (parent.flags & NodeFlags.Static) !== 0 &&
                            parent.parent.kind === TokenType.ClassExpression ? parent.parent : undefined;
                    }
                }
                return undefined;
            }
            ...
            function needsParenthesisForAwaitExpressionAsYield(node: AwaitExpression) {
                if (node.parent.kind === TokenType.BinaryExpression && !isAssignmentOperator((<BinaryExpression>node.parent).operatorToken.kind)) {
                    return true;
                }
                else if (node.parent.kind === TokenType.ConditionalExpression && (<ConditionalExpression>node.parent).condition === node) {
                    return true;
                }
            ...
            function emitListWithSpread(elements: Expression[], needsUniqueCopy: boolean, multiLine: boolean, trailingComma: boolean, useConcat: boolean) {
                ...
                while (pos < length) {
                    ...
                    let e = elements[pos];
                    if (e.kind === TokenType.SpreadElementExpression) {
                        e = (<SpreadElementExpression>e).expression;
                        emitParenthesizedIf(e, /*parenthesized*/ group === 0 && needsParenthesisForPropertyAccessOrInvocation(e));
                        pos++;
                        if (pos === length && group === 0 && needsUniqueCopy && e.kind !== TokenType.ArrayLiteralExpression) {
                            write(".slice()");
                        }
                    }
                    else {
                        let i = pos;
                        while (i < length && elements[i].kind !== TokenType.SpreadElementExpression) {
                            i++;
                        }
                        write("[");
                        if (multiLine) {
            ...
            function isSpreadElementExpression(node: Node) {
                return node.kind === TokenType.SpreadElementExpression;
            }
            ...
            function emitDownlevelObjectLiteralWithComputedProperties(node: ObjectLiteralExpression, firstComputedPropertyIndex: number) {
                ...
                for (let i = firstComputedPropertyIndex, n = properties.length; i < n; i++) {
                    ...
                    emitStart(property);
                    if (property.kind === TokenType.GetAccessor || property.kind === TokenType.SetAccessor) {
                        ...
                        if (property.kind === TokenType.PropertyAssignment) {
                            emit((<PropertyAssignment>property).initializer);
                        }
                        else if (property.kind === TokenType.ShorthandPropertyAssignment) {
                            emitExpressionIdentifier((<ShorthandPropertyAssignment>property).name);
                        }
                        else if (property.kind === TokenType.MethodDeclaration) {
            ...
            function emitObjectLiteral(node: ObjectLiteralExpression): void {
                ...
                if (languageVersion < ScriptTarget.ES6) {
                    ...
                    for (let i = 0, n = properties.length; i < n; i++) {
                        if (properties[i].name.kind === TokenType.ComputedPropertyName) {
            ...
            function createBinaryExpression(left: Expression, operator: TokenType, right: Expression, startsOnNewLine?: boolean): BinaryExpression {
                const result = <BinaryExpression>createSynthesizedNode(TokenType.BinaryExpression, startsOnNewLine);
            ...
            function createPropertyAccessExpression(expression: Expression, name: Identifier): PropertyAccessExpression {
                const result = <PropertyAccessExpression>createSynthesizedNode(TokenType.PropertyAccessExpression);
            ...
            function createElementAccessExpression(expression: Expression, argumentExpression: Expression): ElementAccessExpression {
                const result = <ElementAccessExpression>createSynthesizedNode(TokenType.ElementAccessExpression);
                result.expression = parenthesizeForAccess(expression);
            ...
            function parenthesizeForAccess(expr: Expression): LeftHandSideExpression {
                ...
                while (expr.kind === TokenType.TypeAssertionExpression ||
                    expr.kind === TokenType.AsExpression ||
                    expr.kind === TokenType.NonNullExpression) {
                ...
                if (isLeftHandSideExpression(expr) &&
                    expr.kind !== TokenType.NewExpression &&
                    expr.kind !== TokenType.NumericLiteral) {

                    return <LeftHandSideExpression>expr;
                }
                const node = <ParenthesizedExpression>createSynthesizedNode(TokenType.ParenthesizedExpression);
            ...
            function isImportedReference(node: Identifier) {
                const declaration = resolver.getReferencedImportDeclaration(node);
                return declaration && (declaration.kind === TokenType.ImportClause || declaration.kind === TokenType.ImportSpecifier);
            }
            ...
            function tryEmitConstantValue(node: PropertyAccessExpression | ElementAccessExpression): boolean {
                ...
                if (constantValue !== undefined) {
                    ...
                    if (!compilerOptions.removeComments) {
                        const propertyName: string = node.kind === TokenType.PropertyAccessExpression ? declarationNameToString((<PropertyAccessExpression>node).name) : getTextOfNode((<ElementAccessExpression>node).argumentExpression);
                        write(" /* " + propertyName + " */");
                    }
                    return true;
                }
            ...
            function tryGetConstEnumValue(node: Node): number {
                ...
                return node.kind === TokenType.PropertyAccessExpression || node.kind === TokenType.ElementAccessExpression
            ...
            function emitQualifiedNameAsExpression(node: QualifiedName, useFallback: boolean) {
                if (node.left.kind === TokenType.Identifier) {
            ...
            function hasSpreadElement(elements: Expression[]) {
                return forEach(elements, e => e.kind === TokenType.SpreadElementExpression);
            }

            function skipParentheses(node: Expression): Expression {
                while (node.kind === TokenType.ParenthesizedExpression ||
                    node.kind === TokenType.TypeAssertionExpression ||
                    node.kind === TokenType.AsExpression ||
                    node.kind === TokenType.NonNullExpression) {
            ...
            function emitParenExpression(node: ParenthesizedExpression) {
                ...
                // there.
                if (!nodeIsSynthesized(node) && node.parent.kind !== TokenType.ArrowFunction) {
                    if (node.expression.kind === TokenType.TypeAssertionExpression ||
                        node.expression.kind === TokenType.AsExpression ||
                        node.expression.kind === TokenType.NonNullExpression) {
                        let operand = (<TypeAssertion | NonNullExpression>node.expression).expression;

                        // Make sure we consider all nested cast expressions, e.g.:
                        // (<any><number><any>-A).x;
                        while (operand.kind === TokenType.TypeAssertionExpression ||
                            operand.kind === TokenType.AsExpression ||
                            operand.kind === TokenType.NonNullExpression) {
                        ...
                        if (operand.kind !== TokenType.PrefixUnaryExpression &&
                            operand.kind !== TokenType.VoidExpression &&
                            operand.kind !== TokenType.TypeOfExpression &&
                            operand.kind !== TokenType.DeleteExpression &&
                            operand.kind !== TokenType.PostfixUnaryExpression &&
                            operand.kind !== TokenType.NewExpression &&
                            !(operand.kind === TokenType.CallExpression && node.parent.kind === TokenType.NewExpression) &&
                            !(operand.kind === TokenType.FunctionExpression && node.parent.kind === TokenType.CallExpression) &&
                            !(operand.kind === TokenType.NumericLiteral && node.parent.kind === TokenType.PropertyAccessExpression)) {
                            emit(operand);
                            return;
                        }
                    }
            ...
            function isNameOfExportedSourceLevelDeclarationInSystemExternalModule(node: Node): boolean {
                if (!isCurrentFileSystemExternalModule() || node.kind !== TokenType.Identifier || nodeIsSynthesized(node)) {
                    return false;
                }

                const isVariableDeclarationOrBindingElement =
                    node.parent && (node.parent.kind === TokenType.VariableDeclaration || node.parent.kind === TokenType.BindingElement);
            ...
            function isNameOfExportedDeclarationInNonES6Module(node: Node): boolean {
                if (modulekind === ModuleKind.System || node.kind !== TokenType.Identifier || nodeIsSynthesized(node)) {
                    return false;
                }
            ...
            function isSourceFileLevelDeclarationInSystemJsModule(node: Node, isExported: boolean): boolean {
                ...
                while (current) {
                    if (current.kind === TokenType.SourceFile) {
            ...
            function emitExponentiationOperator(node: BinaryExpression) {
                ...
                if (node.operatorToken.kind === TokenType.AsteriskAsteriskEqualsToken) {
                    ...
                    if (isElementAccessExpression(leftHandSideExpression)) {
                        ...
                        synthesizedLHS = <ElementAccessExpression>createSynthesizedNode(TokenType.ElementAccessExpression, /*startsOnNewLine*/ false);
                        ...
                        if (leftHandSideExpression.argumentExpression.kind !== TokenType.NumericLiteral &&
                            leftHandSideExpression.argumentExpression.kind !== TokenType.StringLiteral) {
                        ...
                        shouldEmitParentheses = true;
                        write("(");
                        synthesizedLHS = <PropertyAccessExpression>createSynthesizedNode(TokenType.PropertyAccessExpression, /*startsOnNewLine*/ false);
            ...
            function emitExpressionStatement(node: ExpressionStatement) {
                emitParenthesizedIf(node.expression, /*parenthesized*/ node.expression.kind === TokenType.ArrowFunction);
                write(";");
            }
            ...
            function emitDoStatementWorker(node: DoStatement, loop: ConvertedLoop) {
                ...
                if (node.statement.kind === TokenType.Block) {
                    write(" ");
                }
                else {
                    writeLine();
            ...
            function emitLoop(node: IterationStatement, loopEmitter: (n: IterationStatement, convertedLoop: ConvertedLoop) => void): void {
                ...
                if (!shouldConvert) {
                    ...
                    if (node.parent.kind === TokenType.LabeledStatement) {
            ...
            function convertLoopBody(node: IterationStatement): ConvertedLoop {
                ...
                switch (node.kind) {
                    ...
                    case TokenType.ForOfStatement:
                        ...
                        if (initializer && initializer.kind === TokenType.VariableDeclarationList) {
                ...
                const bodyIsBlock = node.statement.kind === TokenType.Block;
            ...
            function emitForInOrForOfStatement(node: ForInStatement | ForOfStatement) {
                if (languageVersion < ScriptTarget.ES6 && node.kind === TokenType.ForOfStatement) {
            ...
            function emitForInOrForOfStatementWorker(node: ForInStatement | ForOfStatement, loop: ConvertedLoop) {
                ...
                if (node.kind === TokenType.ForInStatement) {
                    write(" in ");
                }
                else {
                    write(" of ");
            ...
            function emitDownLevelForOfStatementWorker(node: ForOfStatement, loop: ConvertedLoop) {
                ...
                const rhsReference = createSynthesizedNode(TokenType.Identifier) as Identifier;
                rhsReference.text = node.expression.kind === TokenType.Identifier ?
            ...
            function emitBreakOrContinueStatement(node: BreakOrContinueStatement) {
                if (convertedLoopState) {
                    ...
                    const jump = node.kind === TokenType.BreakStatement ? Jump.Break : Jump.Continue;
                    ...
                    if (!canUseBreakOrContinue) {
                        ...
                        if (!node.label) {
                            if (node.kind === TokenType.BreakStatement) {
            ...
            function emitCaseOrDefaultClause(node: CaseOrDefaultClause) {
                if (node.kind === TokenType.CaseClause) {
                    write("case ");
            ...
            function getContainingModule(node: Node): ModuleDeclaration {
                do {
                    node = node.parent;
                } while (node && node.kind !== TokenType.ModuleDeclaration);
            ...
            function createVoidZero(): Expression {
                const zero = <LiteralExpression>createSynthesizedNode(TokenType.NumericLiteral);
                zero.text = "0";
                const result = <VoidExpression>createSynthesizedNode(TokenType.VoidExpression);
                result.expression = zero;
                return result;
            }

            function emitEs6ExportDefaultCompat(node: Node) {
                if (node.parent.kind === TokenType.SourceFile) {
                    Debug.assert(!!(node.flags & NodeFlags.Default) || node.kind === TokenType.ExportAssignment);
            ...
            function emitAssignment(name: Identifier, value: Expression, shouldEmitCommaBeforeAssignment: boolean, nodeForSourceMap: Node) {
                ...
                const isVariableDeclarationOrBindingElement =
                    name.parent && (name.parent.kind === TokenType.VariableDeclaration || name.parent.kind === TokenType.BindingElement);
            ...
            function isFirstVariableDeclaration(root: Node) {
                return root.kind === TokenType.VariableDeclaration &&
                    root.parent.kind === TokenType.VariableDeclarationList &&
            ...
            function emitDestructuring(root: BinaryExpression | VariableDeclaration | ParameterDeclaration, isAssignmentExpressionStatement: boolean, value?: Expression) {
                ...
                if (root.kind === TokenType.VariableDeclaration) {
                ...
                else if (root.kind === TokenType.Parameter) {
                    canDefineTempVariablesInPlace = true;
                }

                if (root.kind === TokenType.BinaryExpression) {
                ...
                function ensureIdentifier(expr: Expression, reuseIdentifierExpressions: boolean, sourceMapNode: Node): Expression {
                    if (expr.kind === TokenType.Identifier && reuseIdentifierExpressions) {
                        return expr;
                    }
                ...
                function createNumericLiteral(value: number) {
                    const node = <LiteralExpression>createSynthesizedNode(TokenType.NumericLiteral);
                    node.text = "" + value;
                    return node;
                }
                ...
                function createPropertyAccessForDestructuringProperty(object: Expression, propName: PropertyName): Expression {
                    ...
                    const nameIsComputed = propName.kind === TokenType.ComputedPropertyName;
                    if (nameIsComputed) {
                    ...
                    return !nameIsComputed && index.kind === TokenType.Identifier
                ...
                function createSliceCall(value: Expression, sliceIndex: number): CallExpression {
                    const call = <CallExpression>createSynthesizedNode(TokenType.CallExpression);
                    const sliceIdentifier = <Identifier>createSynthesizedNode(TokenType.Identifier);
                ...
                function emitObjectLiteralAssignment(target: ObjectLiteralExpression, value: Expression, sourceMapNode: Node) {
                    ...
                    for (const p of properties) {
                        if (p.kind === TokenType.PropertyAssignment || p.kind === TokenType.ShorthandPropertyAssignment) {
                            const propName = <Identifier | LiteralExpression>(<PropertyAssignment>p).name;
                            const target = p.kind === TokenType.ShorthandPropertyAssignment ? <ShorthandPropertyAssignment>p : (<PropertyAssignment>p).initializer || propName;
                ...
                function emitArrayLiteralAssignment(target: ArrayLiteralExpression, value: Expression, sourceMapNode: Node) {
                    ...
                    for (let i = 0; i < elements.length; i++) {
                        const e = elements[i];
                        if (e.kind !== TokenType.OmittedExpression) {
                ...
                function emitDestructuringAssignment(target: Expression | ShorthandPropertyAssignment, value: Expression, sourceMapNode: Node) {
                    ...
                    if (target.kind === TokenType.ShorthandPropertyAssignment) {
                    ...
                    if (target.kind === TokenType.ObjectLiteralExpression) {
                        emitObjectLiteralAssignment(<ObjectLiteralExpression>target, value, sourceMapNode);
                    }
                    else if (target.kind === TokenType.ArrayLiteralExpression) {
                ...
                function emitAssignmentExpression(root: BinaryExpression) {
                    ...
                    if (isEmptyObjectLiteralOrArrayLiteral(target)) {
                        ...
                        if (root.parent.kind !== TokenType.ParenthesizedExpression) {
                            write("(");
                        }
                ...
                function emitBindingElement(target: BindingElement | VariableDeclaration, value: Expression) {
                    ...
                    if (isBindingPattern(target.name)) {
                        ...
                        for (let i = 0; i < numElements; i++) {
                            ...
                            if (pattern.kind === TokenType.ObjectBindingPattern) {
                                // Rewrite element to a declaration with an initializer that fetches property
                                const propName = element.propertyName || <Identifier>element.name;
                                emitBindingElement(element, createPropertyAccessForDestructuringProperty(value, propName));
                            }
                            else if (element.kind !== TokenType.OmittedExpression) {
            ...
            function emitVariableDeclaration(node: VariableDeclaration) {
                if (isBindingPattern(node.name)) {
                    ...
                    if (!initializer &&
                        ...
                        node.name.kind === TokenType.Identifier) {
                        ...
                        const emittedAsTopLevel =
                            ...
                            (isCapturedInFunction && isDeclaredInLoop && container.kind === TokenType.Block && isIterationStatement(container.parent, /*lookInLabeledStatements*/ false));
                        ...
                        const emitExplicitInitializer =
                            ...
                            container.kind !== TokenType.ForInStatement &&
                            container.kind !== TokenType.ForOfStatement &&
                            (
            ...
            function emitExportVariableAssignments(node: VariableDeclaration | BindingElement) {
                if (node.kind === TokenType.OmittedExpression) {
                    return;
                }
            ...
            function isES6ExportedDeclaration(node: Node) {
                return !!(node.flags & NodeFlags.Export) &&
                    ...
                    node.parent.kind === TokenType.SourceFile;
            }
            ...
            function emitAccessor(node: AccessorDeclaration) {
                write(node.kind === TokenType.GetAccessor ? "get " : "set ");
                emit(node.name);
                emitSignatureAndBody(node);
            }

            function shouldEmitAsArrowFunction(node: FunctionLikeDeclaration): boolean {
                return node.kind === TokenType.ArrowFunction && languageVersion >= ScriptTarget.ES6;
            }
            ...
            function shouldEmitFunctionName(node: FunctionLikeDeclaration) {
                if (node.kind === TokenType.FunctionExpression) {
                    // Emit name if one is present
                    return !!node.name;
                }
                if (node.kind === TokenType.FunctionDeclaration) {
            ...
            function emitFunctionDeclaration(node: FunctionLikeDeclaration) {
                ...
                if (kind !== TokenType.MethodDeclaration &&
                    kind !== TokenType.MethodSignature &&
                    parent &&
                    parent.kind !== TokenType.PropertyAssignment &&
                    parent.kind !== TokenType.CallExpression &&
                    parent.kind !== TokenType.ArrayLiteralExpression) {
                ...
                emitSignatureAndBody(node);
                if (modulekind !== ModuleKind.ES6 && kind === TokenType.FunctionDeclaration && parent === currentSourceFile && node.name) {
            ...
            function emitAsyncFunctionBodyForES6(node: FunctionLikeDeclaration) {
                ...
                const isArrowFunction = node.kind === TokenType.ArrowFunction;
...
const _super = (function (geti, seti) {
    ...
    return name => cache[name] || (cache[name] = { get value() { return geti(name); }, set value(v) { seti(name, v); } });
            ...
            function emitExpressionFunctionBody(node: FunctionLikeDeclaration, body: Expression) {
                ...
                let current = body;
                while (current.kind === TokenType.TypeAssertionExpression) {
                    current = (<TypeAssertion>current).expression;
                }

                emitParenthesizedIf(body, current.kind === TokenType.ObjectLiteralExpression);
            }
            ...
            function getSuperCallAtGivenIndex(ctor: ConstructorDeclaration, index: number): ExpressionStatement {
                ...
                if (statement.kind === TokenType.ExpressionStatement) {
            ...
            function emitMemberAccessForPropertyName(memberName: DeclarationName) {
                ...
                if (memberName.kind === TokenType.StringLiteral || memberName.kind === TokenType.NumericLiteral) {
                    write("[");
                    emitNodeWithCommentsAndWithoutSourcemap(memberName);
                    write("]");
                }
                else if (memberName.kind === TokenType.ComputedPropertyName) {
            ...
            function getInitializedProperties(node: ClassLikeDeclaration, isStatic: boolean) {
                ...
                for (const member of node.members) {
                    if (member.kind === TokenType.PropertyDeclaration && isStatic === ((member.flags & NodeFlags.Static) !== 0) && (<PropertyDeclaration>member).initializer) {
            ...
            function emitMemberFunctionsForES5AndLower(node: ClassLikeDeclaration) {
                forEach(node.members, member => {
                    if (member.kind === TokenType.SemicolonClassElement) {
                        writeLine();
                        write(";");
                    }
                    else if (member.kind === TokenType.MethodDeclaration || node.kind === TokenType.MethodSignature) {
                    ...
                    else if (member.kind === TokenType.GetAccessor || member.kind === TokenType.SetAccessor) {
            ...
            function emitMemberFunctionsForES6AndHigher(node: ClassLikeDeclaration) {
                for (const member of node.members) {
                    if ((member.kind === TokenType.MethodDeclaration || node.kind === TokenType.MethodSignature) && !(<MethodDeclaration>member).body) {
                        emitCommentsOnNotEmittedNode(member);
                    }
                    else if (member.kind === TokenType.MethodDeclaration ||
                        member.kind === TokenType.GetAccessor ||
                        member.kind === TokenType.SetAccessor) {
                        writeLine();
                        emitLeadingComments(member);
                        emitStart(member);
                        ...
                        if (member.kind === TokenType.GetAccessor) {
                            write("get ");
                        }
                        else if (member.kind === TokenType.SetAccessor) {
                            write("set ");
                        }
                    ...
                    else if (member.kind === TokenType.SemicolonClassElement) {
                        writeLine();
                        write(";");
                    }
                }
            ...
            function emitConstructorWorker(node: ClassLikeDeclaration, baseTypeElement: ExpressionWithTypeArguments) {
                ...
                forEach(node.members, member => {
                    if (member.kind === TokenType.Constructor && !(<ConstructorDeclaration>member).body) {
                        emitCommentsOnNotEmittedNode(member);
                    }
                    // Check if there is any non-static property assignment
                    if (member.kind === TokenType.PropertyDeclaration && (<PropertyDeclaration>member).initializer && (member.flags & NodeFlags.Static) === 0) {
            ...
            function emitClassLikeDeclarationForES6AndHigher(node: ClassLikeDeclaration) {
                ...
                if (node.kind === TokenType.ClassDeclaration) {
                ...
                const isClassExpressionWithStaticProperties = staticProperties.length > 0 && node.kind === TokenType.ClassExpression;
                let generatedName: string;

                if (isClassExpressionWithStaticProperties) {
                    generatedName = getGeneratedNameForNode(node.name);
                    const synthesizedNode = <Identifier>createSynthesizedNode(TokenType.Identifier);
                ...
                if (modulekind !== ModuleKind.ES6) {
                    ...
                    else if (node.parent.kind !== TokenType.SourceFile) {
                        writeLine();
                        emitStart(node);
                        emitModuleMemberName(node);
                        write(" = ");
            ...
            function emitClassLikeDeclarationBelowES6(node: ClassLikeDeclaration) {
                ...
                if (node.kind === TokenType.ClassDeclaration && !isES6ExportedClass) {
            ...
            function emitDecoratorsOfMembers(node: ClassLikeDeclaration, staticFlag: NodeFlags) {
                for (const member of node.members) {
                    ...
                    if (isAccessor(member)) {
                        ...
                        if (member.kind === TokenType.MethodDeclaration) {
                    ...
                    if (languageVersion > ScriptTarget.ES3) {
                        if (member.kind !== TokenType.PropertyDeclaration) {
                        ...
                        if (parameterCount > 0) {
                            for (let i = 0; i < parameterCount; i++) {
                                ...
                                if (parameters[i].dotDotDotToken) {
                                    ...
                                    if (parameterType && parameterType.kind === TokenType.ArrayType) {
                                        parameterType = (<ArrayTypeNode>parameterType).elementType;
                                    }
                                    else if (parameterType && parameterType.kind === TokenType.TypeReference && (<TypeReferenceNode>parameterType).typeArguments && (<TypeReferenceNode>parameterType).typeArguments.length === 1) {
            ...
            function emitEnumDeclaration(node: EnumDeclaration) {
                ...
                if (!shouldHoistDeclarationInSystemJsModule(node)) {
                    ...
                    if (!(node.flags & NodeFlags.Export) || (isES6ExportedEnum && isFirstDeclarationOfKind(node, node.symbol && node.symbol.declarations, TokenType.EnumDeclaration))) {
                        emitStart(node);
            ...
            function getInnerMostModuleDeclarationFromDottedModule(moduleDeclaration: ModuleDeclaration): ModuleDeclaration {
                if (moduleDeclaration.body && moduleDeclaration.body.kind === TokenType.ModuleDeclaration) {
            ...
            function isFirstDeclarationOfKind(node: Declaration, declarations: Declaration[], kind: TokenType) {
            ...
            function emitModuleDeclaration(node: ModuleDeclaration) {
                ...
                if (emitVarForModule) {
                    ...
                    if (!isES6ExportedNamespace || isFirstDeclarationOfKind(node, node.symbol && node.symbol.declarations, TokenType.ModuleDeclaration)) {
                        emitStart(node);
                ...
                if (node.body.kind === TokenType.ModuleBlock) {
                    const saveConvertedLoopState = convertedLoopState;
                    const saveTempFlags = tempFlags;
                    const saveTempVariables = tempVariables;
                    convertedLoopState = undefined;
                ...
                write(" = {}));");
                emitEnd(node);
                if (!isES6ExportedDeclaration(node) && node.name.kind === TokenType.Identifier && node.parent === currentSourceFile) {
                    if (modulekind === ModuleKind.System && (node.flags & NodeFlags.Export)) {
                        writeLine();
                        write(`${exportFunctionForFile}("`);
                        emitDeclarationName(node);
            ...
            function emitRequire(moduleName: Expression) {
                if (moduleName.kind === TokenType.StringLiteral) {
                    write("require(");
            ...
            function getNamespaceDeclarationNode(node: ImportDeclaration | ImportEqualsDeclaration | ExportDeclaration) {
                ...
                if (importClause && importClause.namedBindings && importClause.namedBindings.kind === TokenType.NamespaceImport) {
            ...
            function isDefaultImport(node: ImportDeclaration | ImportEqualsDeclaration | ExportDeclaration) {
                return node.kind === TokenType.ImportDeclaration && (<ImportDeclaration>node).importClause && !!(<ImportDeclaration>node).importClause.name;
            }
                ...
                if (node.importClause) {
                    ...
                    if (shouldEmitDefaultBindings || shouldEmitNamedBindings) {
                        ...
                        if (shouldEmitNamedBindings) {
                            ...
                            if (node.importClause.namedBindings.kind === TokenType.NamespaceImport) {
                                write("* as ");
            ...
            function emitExternalImportDeclaration(node: ImportDeclaration | ImportEqualsDeclaration) {
                if (contains(externalImports, node)) {
                    const isExportedImport = node.kind === TokenType.ImportEqualsDeclaration && (node.flags & NodeFlags.Export) !== 0;
                    ...
                    if (modulekind !== ModuleKind.AMD) {
                        ...
                        if (namespaceDeclaration && !isDefaultImport(node)) {
                            ...
                            const isNakedImport = TokenType.ImportDeclaration && !(<ImportDeclaration>node).importClause;
                            if (!isNakedImport) {
                                write(varOrConst);
            ...
            function emitExportAssignment(node: ExportAssignment) {
                if (!node.isExportEquals && resolver.isValueAliasDeclaration(node)) {
                    if (modulekind === ModuleKind.ES6) {
                        ...
                        const expression = node.expression;
                        emit(expression);
                        if (expression.kind !== TokenType.FunctionDeclaration &&
                            expression.kind !== TokenType.ClassDeclaration) {
                            write(";");
                        }
                        emitEnd(node);
                    }
                ...
                for (const node of sourceFile.statements) {
                    switch (node.kind) {
                        case TokenType.ImportDeclaration:
                            ...
                            break;
                        case TokenType.ImportEqualsDeclaration:
                            if ((<ImportEqualsDeclaration>node).moduleReference.kind === TokenType.ExternalModuleReference && resolver.isReferencedAliasDeclaration(node)) {
            ...
            function getLocalNameForExternalImport(node: ImportDeclaration | ExportDeclaration | ImportEqualsDeclaration): string {
                ...
                if (node.kind === TokenType.ImportDeclaration && (<ImportDeclaration>node).importClause) {
                    return getGeneratedNameForNode(node);
                }
                if (node.kind === TokenType.ExportDeclaration && (<ExportDeclaration>node).moduleSpecifier) {
                    return getGeneratedNameForNode(node);
                }
            }
            ...
            function emitVariableDeclarationsForImports(): void {
                ...
                for (const importNode of externalImports) {
                    ...
                    const skipNode =
                        importNode.kind === TokenType.ExportDeclaration ||
                        (importNode.kind === TokenType.ImportDeclaration && !(<ImportDeclaration>importNode).importClause);
            ...
            function emitLocalStorageForExportedNamesIfNecessary(exportedDeclarations: (Identifier | Declaration)[]): string {
                ...
                if (!exportedDeclarations && isEmpty(exportSpecifiers)) {
                    ...
                    for (const externalImport of externalImports) {
                        if (externalImport.kind === TokenType.ExportDeclaration && (<ExportDeclaration>externalImport).exportClause) {
                ...
                for (const externalImport of externalImports) {
                    if (externalImport.kind !== TokenType.ExportDeclaration) {
                        continue;
                    }
                ...
                function writeExportedName(node: Identifier | Declaration): void {
                    ...
                    if (node.kind !== TokenType.Identifier && node.flags & NodeFlags.Default) {
                        return;
                    }
            ...
            function processTopLevelVariableAndFunctionDeclarations(node: SourceFile): (Identifier | Declaration)[] {
                ...
                if (hoistedVars) {
                    ...
                    for (let i = 0; i < hoistedVars.length; i++) {
                        ...
                        const name = local.kind === TokenType.Identifier
                            ? <Identifier>local
                        ...
                        if (local.kind === TokenType.ClassDeclaration || local.kind === TokenType.ModuleDeclaration || local.kind === TokenType.EnumDeclaration) {
                        ...
                        const flags = getCombinedNodeFlags(local.kind === TokenType.Identifier ? local.parent : local);
                ...
                function visit(node: Node): void {
                    ...
                    if (node.kind === TokenType.EnumDeclaration) {
                    ...
                    if (node.kind === TokenType.ModuleDeclaration) {
                    ...
                    if (node.kind === TokenType.VariableDeclaration || node.kind === TokenType.BindingElement) {
            ...
            function shouldHoistVariable(node: VariableDeclaration | VariableDeclarationList | BindingElement, checkIfSourceFileLevelDecl: boolean): boolean {
                ...
                return (getCombinedNodeFlags(node) & NodeFlags.BlockScoped) === 0 ||
                    getEnclosingBlockScopeContainer(node).kind === TokenType.SourceFile;
            }
    ...
    if (typeof module === 'object' && typeof module.exports === 'object') {
        ...
        define(`);
            ...
            function isJsxChildEmittable(child: JsxChild): boolean  {
                if (child.kind === TokenType.JsxExpression) {
                    // Don't emit empty expressions
                    return !!(<JsxExpression>child).expression;

                }
                else if (child.kind === TokenType.JsxText) {
            ...
            function shouldEmitLeadingAndTrailingComments(node: Node) {
                ...
                if (node.kind !== TokenType.Block &&
                    node.parent &&
                    node.parent.kind === TokenType.ArrowFunction &&
            ...
            function isTripleSlashComment(comment: CommentRange) {
                ...
                if (currentText.charCodeAt(comment.pos + 1) === CharCode.slash &&
                    comment.pos + 2 < comment.end &&
                    currentText.charCodeAt(comment.pos + 2) === CharCode.slash) {
            ...
            function getLeadingCommentsToEmit(node: Node) {
                ...
                if (node.parent) {
                    if (node.parent.kind === TokenType.SourceFile || node.pos !== node.parent.pos) {
            ...
            function getTrailingCommentsToEmit(node: Node) {
                ...
                if (node.parent) {
                    if (node.parent.kind === TokenType.SourceFile || node.end !== node.parent.end) {
