    ...
    function emitDeclarations(host: EmitHost, resolver: EmitResolver, emitterDiagnostics: DiagnosticCollection, declarationFilePath: string,
            ...
            if (moduleElementDeclarationEmitInfo.length) {
                ...
                forEach(moduleElementDeclarationEmitInfo, aliasEmitInfo => {
                    if (aliasEmitInfo.isVisible && !aliasEmitInfo.asynchronousOutput) {
                        Debug.assert(aliasEmitInfo.node.kind === TokenType.ImportDeclaration);
                        createAndSetNewTextWriterWithSymbolWriter();
        ...
        function writeAsynchronousModuleElements(nodes: Node[]) {
            ...
            forEach(nodes, declaration => {
                ...
                if (declaration.kind === TokenType.VariableDeclaration) {
                    nodeToCheck = declaration.parent.parent;
                }
                else if (declaration.kind === TokenType.NamedImports || declaration.kind === TokenType.ImportSpecifier || declaration.kind === TokenType.ImportClause) {
                ...
                if (moduleElementEmitInfo) {
                    if (moduleElementEmitInfo.node.kind === TokenType.ImportDeclaration) {
                        ...
                        if (nodeToCheck.kind === TokenType.ModuleDeclaration) {
        ...
        function emitType(type: TypeNode | Identifier | QualifiedName) {
            ...
            function writeEntityName(entityName: EntityName | Expression) {
                if (entityName.kind === TokenType.Identifier) {
                    writeTextOfNode(currentText, entityName);
                }
                else {
                    const left = entityName.kind === TokenType.QualifiedName ? (<QualifiedName>entityName).left : (<PropertyAccessExpression>entityName).expression;
                    const right = entityName.kind === TokenType.QualifiedName ? (<QualifiedName>entityName).right : (<PropertyAccessExpression>entityName).name;
            ...
            function emitEntityName(entityName: EntityName | PropertyAccessExpression) {
                const visibilityResult = resolver.isEntityNameVisible(entityName,
                    ...
                    entityName.parent.kind === TokenType.ImportEqualsDeclaration ? entityName.parent : enclosingDeclaration);
            ...
            function emitExpressionWithTypeArguments(node: ExpressionWithTypeArguments) {
                if (isSupportedExpressionWithTypeArguments(node)) {
                    Debug.assert(node.expression.kind === TokenType.Identifier || node.expression.kind === TokenType.PropertyAccessExpression);
        ...
        function emitExportAssignment(node: ExportAssignment) {
            if (node.expression.kind === TokenType.Identifier) {
        ...
        function emitModuleElement(node: Node, isModuleElementVisible: boolean) {
            ...
            else if (node.kind === TokenType.ImportEqualsDeclaration ||
                (node.parent.kind === TokenType.SourceFile && isCurrentFileExternalModule)) {
                let isVisible: boolean;
                if (asynchronousSubModuleDeclarationEmitInfo && node.parent.kind !== TokenType.SourceFile) {
        ...
        function emitModuleElementDeclarationFlags(node: Node) {
            ...
            if (node.parent.kind === TokenType.SourceFile) {
                ...
                else if (node.kind !== TokenType.InterfaceDeclaration && !noDeclare) {
                    write("declare ");
                }
            }
        }
        ...
        function isVisibleNamedBinding(namedBindings: NamespaceImport | NamedImports): boolean {
            if (namedBindings) {
                if (namedBindings.kind === TokenType.NamespaceImport) {
        ...
        function emitExternalModuleSpecifier(parent: ImportEqualsDeclaration | ImportDeclaration | ExportDeclaration | ModuleDeclaration) {
            ...
            resultHasExternalModuleIndicator = resultHasExternalModuleIndicator || parent.kind !== TokenType.ModuleDeclaration;
            let moduleSpecifier: Node;
            if (parent.kind === TokenType.ImportEqualsDeclaration) {
                const node = parent as ImportEqualsDeclaration;
                moduleSpecifier = getExternalModuleImportEqualsDeclarationExpression(node);
            }
            else if (parent.kind === TokenType.ModuleDeclaration) {
            ...
            if (moduleSpecifier.kind === TokenType.StringLiteral && isBundledEmit && (compilerOptions.out || compilerOptions.outFile)) {
        ...
        function writeModuleDeclaration(node: ModuleDeclaration) {
            ...
            while (node.body && node.body.kind !== TokenType.ModuleBlock) {
        ...
        function isPrivateMethodTypeParameter(node: TypeParameterDeclaration) {
            return node.parent.kind === TokenType.MethodDeclaration && (node.parent.flags & NodeFlags.Private);
        }
        ...
        function emitTypeParameters(typeParameters: TypeParameterDeclaration[]) {
            function emitTypeParameter(node: TypeParameterDeclaration) {
                ...
                if (node.constraint && !isPrivateMethodTypeParameter(node)) {
                    write(" extends ");
                    if (node.parent.kind === TokenType.FunctionType ||
                        node.parent.kind === TokenType.ConstructorType ||
                        (node.parent.parent && node.parent.parent.kind === TokenType.TypeLiteral)) {
                        Debug.assert(node.parent.kind === TokenType.MethodDeclaration ||
                            node.parent.kind === TokenType.MethodSignature ||
                            node.parent.kind === TokenType.FunctionType ||
                            node.parent.kind === TokenType.ConstructorType ||
                            node.parent.kind === TokenType.CallSignature ||
                            node.parent.kind === TokenType.ConstructSignature);
                ...
                function getTypeParameterConstraintVisibilityError(symbolAccessibilityResult: SymbolAccessibilityResult): SymbolAccessibilityDiagnostic {
                    ...
                    switch (node.parent.kind) {
                        ...
                        case TokenType.MethodSignature:
                            ...
                            else if (node.parent.parent.kind === TokenType.ClassDeclaration) {
        ...
        function emitHeritageClause(typeReferences: ExpressionWithTypeArguments[], isImplementsList: boolean) {
            ...
            function emitTypeOfTypeReference(node: ExpressionWithTypeArguments) {
                ...
                function getHeritageClauseVisibilityError(symbolAccessibilityResult: SymbolAccessibilityResult): SymbolAccessibilityDiagnostic {
                    ...
                    if (node.parent.parent.kind === TokenType.ClassDeclaration) {
        ...
        function emitVariableDeclaration(node: VariableDeclaration) {
            ...
            if (node.kind !== TokenType.VariableDeclaration || resolver.isDeclarationVisible(node)) {
                if (isBindingPattern(node.name)) {
                    ...
                    if ((node.kind === TokenType.PropertyDeclaration || node.kind === TokenType.PropertySignature || node.kind === TokenType.Parameter) && hasQuestionToken(node)) {
                        write("?");
                    }
                    if ((node.kind === TokenType.PropertyDeclaration || node.kind === TokenType.PropertySignature) && node.parent.kind === TokenType.TypeLiteral) {
            ...
            function getVariableDeclarationTypeVisibilityDiagnosticMessage(symbolAccessibilityResult: SymbolAccessibilityResult) {
                ...
                // This check is to ensure we don't report error on constructor parameter property as that error would be reported during parameter emit
                else if (node.kind === TokenType.PropertyDeclaration || node.kind === TokenType.PropertySignature) {
                    ...
                    else if (node.parent.kind === TokenType.ClassDeclaration) {
                        return symbolAccessibilityResult.errorModuleName ?
                            symbolAccessibilityResult.accessibility === SymbolAccessibility.CannotBeNamed ?
            ...
            function emitBindingPattern(bindingPattern: BindingPattern) {
                ...
                for (const element of bindingPattern.elements) {
                    if (element.kind !== TokenType.OmittedExpression) {
        ...
        function emitAccessorDeclaration(node: AccessorDeclaration) {
            ...
            if (node === accessors.firstAccessor) {
                ...
                if (!(node.flags & NodeFlags.Private)) {
                    ...
                    if (!type) {
                        ...
                        const anotherAccessor = node.kind === TokenType.GetAccessor ? accessors.setAccessor : accessors.getAccessor;
            ...
            function getTypeAnnotationFromAccessor(accessor: AccessorDeclaration): TypeNode {
                if (accessor) {
                    return accessor.kind === TokenType.GetAccessor
            ...
            function getAccessorDeclarationTypeVisibilityError(symbolAccessibilityResult: SymbolAccessibilityResult): SymbolAccessibilityDiagnostic {
                let diagnosticMessage: DiagnosticMessage;
                if (accessorWithTypeAnnotation.kind === TokenType.SetAccessor) {
        ...
        function writeFunctionDeclaration(node: FunctionLikeDeclaration) {
            ...
            if (!resolver.isImplementationOfOverload(node)) {
                ...
                else if (node.kind === TokenType.MethodDeclaration || node.kind === TokenType.Constructor) {
                    emitClassMemberDeclarationFlags(node.flags);
                }
                if (node.kind === TokenType.FunctionDeclaration) {
                    write("function ");
                ...
                else if (node.kind === TokenType.Constructor) {
                    write("constructor");
                }
                else {
                    writeTextOfNode(currentText, node.name);
        ...
        function emitSignatureDeclaration(node: SignatureDeclaration) {
            ...
            if (node.kind === TokenType.IndexSignature) {
                ...
                if (node.kind === TokenType.ConstructSignature || node.kind === TokenType.ConstructorType) {
                    write("new ");
                }
                else if (node.kind === TokenType.FunctionType) {
            ...
            const isFunctionTypeOrConstructorType = node.kind === TokenType.FunctionType || node.kind === TokenType.ConstructorType;
            if (isFunctionTypeOrConstructorType || node.parent.kind === TokenType.TypeLiteral) {
            ...
            else if (node.kind !== TokenType.Constructor && !(node.flags & NodeFlags.Private)) {
        ...
        function emitParameterDeclaration(node: ParameterDeclaration) {
            ...
            if (node.parent.kind === TokenType.FunctionType ||
                node.parent.kind === TokenType.ConstructorType ||
                node.parent.parent.kind === TokenType.TypeLiteral) {
                emitTypeOfVariableDeclarationFromTypeLiteral(node);
            }
            ...
            function emitBindingElement(bindingElement: BindingElement) {
                ...
                if (bindingElement.kind === TokenType.OmittedExpression) {
                ...
                else if (bindingElement.kind === TokenType.BindingElement) {
                    ...
                    if (bindingElement.name) {
                        if (isBindingPattern(bindingElement.name)) {
                            ...
                            Debug.assert(bindingElement.name.kind === TokenType.Identifier);
